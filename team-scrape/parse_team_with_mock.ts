...
// Enhanced version of parse_team.ts with mock Fanspo support
//
// This script extends the original parser to support mock Fanspo data
// when the FANSPO_MOCK=1 environment variable is set
//
// Usage:
...
async function fetchFanspoTeamPicks(
  ...
  if (useMock) {
    // Use mock data for testing without network access
    const mockHtml = getMockFanspoResponse(teamSlug, teamId);
    if (!mockHtml) {
      throw new Error(`No mock data available for ${teamSlug}-${teamId}`);
    }
    html = mockHtml;
...
async function main() {
  ...
  const FANSPO_MOCK = process.env.FANSPO_MOCK === '1'; // NEW: Mock mode
