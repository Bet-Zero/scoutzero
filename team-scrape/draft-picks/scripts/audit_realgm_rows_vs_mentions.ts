...
const OBS_MENTIONS_DIR = path.join(
  BASE_DIR,
  'team-scrape/draft-picks/_artifacts/output/mentions'
);
const OBS_LEDGER_DIR = path.join(
  BASE_DIR,
...
function parseCompressedPickData(
  ...
  let text = compressedText
    .replace(/Year(First|Second)\s*Round\s*Picks/gi, '')
    .replace(/Loading,\s*please\s*wait/gi, '');
  text = norm(text);
  ...
  if (firstHeader) {
    ...
    if (t.length > 0) {
      ...
      $siblings.each((_, el) => {
        ...
        if (
          txt.includes('YearFirst Round Picks') ||
          (txt.includes('Year') && txt.includes('Own'))
        ) {
  ...
  if (secondHeader) {
    ...
    if (t.length > 0) {
      ...
      $siblings.each((_, el) => {
        const txt = $(el).text().trim();
        if (
          txt.includes('YearSecond Round Picks') ||
          (txt.includes('Year') && txt.includes('To'))
        ) {
          rows.push(...parseCompressedPickData(txt));
        }
      });
...
async function auditTeam(teamCode: string) {
  ...
  const mentionsPath = path.join(
    OBS_MENTIONS_DIR,
    `draft_picks_mentions_${teamCode}.json`
  );
  let mentions: any[] = [];
  try {
  ...
  for (const row of realRows) {
    ...
    const bucketHasAnyRaw = bucketMentions.some(
      (m) => m.metadata?.realgmRawText
    );
  ...
  try {
    ...
    const checkDupes = (arr: any[], name: string) => {
      ...
      const dupes = ids.filter((id: string) =>
        output.has(id) ? true : (output.add(id), false)
      );
      if (dupes.length)
        ledgerIssues.push(`${name} has duplicates: ${dupes.join(', ')}`);
    };
    ...
    (inventory || []).forEach((p: any) => {
      if (p.status === 'contested')
        ledgerIssues.push(`Inventory includes contested pick: ${p.id}`);
    });
    ...
    allPicks.forEach((p: any) => {
      ...
      if (!VALID_CODES.has(normOrig))
        ledgerIssues.push(
          `Invalid originalTeam ${p.originalTeam} (norm: ${normOrig}) in ${p.id}`
        );
      if (!VALID_CODES.has(normOwn))
        ledgerIssues.push(
          `Invalid owner ${p.owner} (norm: ${normOwn}) in ${p.id}`
        );
    });
  } catch (e) {
  ...
  const recursiveScan = (obj: any, path: string) => {
    ...
    if (typeof obj === 'string') {
      if (obj === 'PHO' || obj === 'PHU')
        hygieneIssues.push(`Found banned code "${obj}" at ${path}`);
    } else if (Array.isArray(obj)) {
      obj.forEach((v, i) => recursiveScan(v, `${path}[${i}]`));
    } else if (typeof obj === 'object') {
      for (const k in obj) {
        if (k === 'PHO' || k === 'PHU')
          hygieneIssues.push(`Found banned key "${k}" at ${path}`);
...
async function run() {
  ...
  const teamList = arg
    ? arg.split('=')[1] === 'ALL'
      ? Object.values(INTERNAL_TEAM_CODE_MAP)
      : arg.split('=')[1].split(',')
    : Object.values(INTERNAL_TEAM_CODE_MAP);
      ...
      await Promise.all(
        batch.map(async (t) => {
          ...
          try {
            ...
            const details = [];
            if (res.categoryC.length)
              details.push(`MISSING(C):${res.categoryC.length}`);
            else if (res.categoryA.length || res.categoryB.length)
              details.push(
                `Warn(A/B):${res.categoryA.length + res.categoryB.length}`
              );

            if (res.ledgerIssues.length)
              details.push(`Ledger:${res.ledgerIssues.length}`);
            if (res.hygieneIssues.length)
              details.push(`Hygiene:${res.hygieneIssues.length}`);

            console.log(
              `Scanning ${t}... ${res.status} ${details.length ? '(' + details.join(', ') + ')' : ''}`
            );
          } catch (e) {
    ...
    if (failures.length > 0) {
      ...
      for (const f of failures) {
        ...
        if (f.categoryC?.length) {
          ...
          f.categoryC.forEach((c: any) =>
            console.log(
              `    [${c.year}] "${c.text}" (Best: ${c.bestCandidate} @ ${c.bestScore})`
            )
          );
        }
        if (f.categoryB?.length) {
          console.log('  CATEGORY B (EXTRACTION GAP):');
          f.categoryB.forEach((c: any) =>
            console.log(
              `    [${c.year}] "${c.text}" (Best: ${c.bestCandidate} @ ${c.bestScore})`
            )
          );
        ...
        if (f.categoryA?.length) {
          ...
          f.categoryA
            ...
            .forEach((c: any) =>
              console.log(
                `    [${c.year}] "${c.text}" (BestScore: ${c.bestScore})`
              )
            );
          if (f.categoryA.length > 5)
            console.log(`    ... and ${f.categoryA.length - 5} more`);
        }

        if (f.ledgerIssues?.length)
          console.log('  LEDGER ISSUES:', f.ledgerIssues);
        if (f.hygieneIssues?.length)
          console.log('  HYGIENE ISSUES:', f.hygieneIssues);
      }
    } else {
      console.log('\n✅ ALL SYSTEMS PASS');
    }
