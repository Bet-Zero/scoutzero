...
// Output Files
const OUTPUT_ASSETS_PATH = path.join(
  DATA_DIR,
  'pst_entitlement_assets_2026_2033.json'
);
const OUTPUT_BY_TEAM_PATH = path.join(
  DATA_DIR,
  'pst_entitlements_by_team_2026_2033.json'
);
...
function normalizeComparator(
  order?: 'most' | 'least'
): 'more_favorable' | 'less_favorable' | undefined {
...
function main() {
  ...
  for (const pick of ledger.picks) {
    ...
    for (const spec of specs) {
      ...
      const poolIds = spec.pool.map(
        (team) => `${team}_${year}_${round === 1 ? '1st' : '2nd'}`
      );
        ...
        if (others.length === 2) {
          ...
          if (!seenHashes.has(convId)) {
            ...
            allAssets.push({
              ...
              poolUnderlyingPickIds: others.map(
                (t) => `${t}_${year}_${round === 1 ? '1st' : '2nd'}`
              ),
              receivesRank: [1],
          ...
          const swapHashContent = [
            holder,
            year,
            round,
            'swap_right',
            `target=less(${others.join(',')})`,
          ].join('|');
          ...
          if (!seenHashes.has(swapId)) {
            ...
            allAssets.push({
              ...
              poolUnderlyingPickIds: spec.pool.map(
                (t) => `${t}_${year}_${round === 1 ? '1st' : '2nd'}`
              ),
            });
          }
      ...
      const hashContent = [
        ...
        kind === 'conveyance_right'
          ? `${asset.receivesRank?.join(',')}-${asset.receivesComparator}`
          : 'opt',
      ].join('|');
  ...
  // Phase 8.2: Build swap_right index for encumbered detection
  // A pick is only "encumbered" if a swap_right entitlement references it
  const swapRightsByUnderlyingPick = new Map<string, string[]>();
  for (const asset of allAssets) {
    if (asset.kind !== 'swap_right') continue;

    // Index by swapControllerPickId
    if (asset.swapControllerPickId) {
      if (!swapRightsByUnderlyingPick.has(asset.swapControllerPickId)) {
        swapRightsByUnderlyingPick.set(asset.swapControllerPickId, []);
      }
      swapRightsByUnderlyingPick
        .get(asset.swapControllerPickId)!
        .push(asset.id);
    }

    // Index by poolUnderlyingPickIds (if pool-based swap)
    if (asset.poolUnderlyingPickIds) {
      for (const poolPickId of asset.poolUnderlyingPickIds) {
        if (!swapRightsByUnderlyingPick.has(poolPickId)) {
          swapRightsByUnderlyingPick.set(poolPickId, []);
        }
        // Only add if not already present (dedup)
        const existing = swapRightsByUnderlyingPick.get(poolPickId)!;
  ...
  for (const pick of ledger.picks) {
    ...
    const coveredByConveyances = allAssets.filter(
      (asset) =>
    ...
    // Phase 8.2: Check if any swap_right entitlement references this pick
    const swapBackingEntitlementIds =
      swapRightsByUnderlyingPick.get(pickId) || [];

    let underlyingStatus: 'pooled' | 'encumbered' | 'clean' = 'clean';
    let coveredByEntitlementIds: string[] | undefined = undefined;

    if (coveredByConveyances.length > 0) {
      underlyingStatus = 'pooled';
      coveredByEntitlementIds = coveredByConveyances.map((e) => e.id).sort();
    } else if (swapBackingEntitlementIds.length > 0) {
      // Phase 8.2: Only mark encumbered if swap_right actually references this pick
      underlyingStatus = 'encumbered';
      coveredByEntitlementIds = [...swapBackingEntitlementIds].sort();
    }
    // Otherwise: clean (no pooling, no swap backing)
    ...
    if (!seenHashes.has(id)) {
      ...
      allAssets.push({
        ...
        underlyingStatus,
        coveredByEntitlementIds: coveredByEntitlementIds,
      } as EntitlementAsset);
    }
  }

  // Phase 8.2: Invariant Validator - No encumbered pick_ownership without swap backing
  console.log('\n=== Phase 8.2 Invariant Validation ===');
  const encumberedWithoutBacking: string[] = [];
  const allSwapRightIds = new Set(
    allAssets.filter((a) => a.kind === 'swap_right').map((a) => a.id)
  );

  for (const asset of allAssets) {
    if (asset.kind !== 'pick_ownership') continue;
    if (asset.underlyingStatus !== 'encumbered') continue;

    // Encumbered pick_ownership MUST have coveredByEntitlementIds populated
    if (
      !asset.coveredByEntitlementIds ||
      asset.coveredByEntitlementIds.length === 0
    ) {
      encumberedWithoutBacking.push(
        `${asset.id} (underlyingPickId: ${asset.underlyingPickId}) - missing coveredByEntitlementIds`
      );
      continue;
    }

    // All coveredByEntitlementIds MUST be valid swap_right entitlements
    for (const coverId of asset.coveredByEntitlementIds) {
      if (!allSwapRightIds.has(coverId)) {
        encumberedWithoutBacking.push(
          `${asset.id} (underlyingPickId: ${asset.underlyingPickId}) - coveredByEntitlementId ${coverId} is not a swap_right`
        );
      }
    }
  }

  if (encumberedWithoutBacking.length > 0) {
    console.error(
      '❌ INVARIANT VIOLATION: Encumbered pick_ownership without swap backing:'
    );
    for (const msg of encumberedWithoutBacking) {
      console.error(`   - ${msg}`);
    }
    process.exit(1);
  }

  const encumberedCount = allAssets.filter(
    (a) => a.kind === 'pick_ownership' && a.underlyingStatus === 'encumbered'
  ).length;
  console.log(
    `✅ Invariant passed: ${encumberedCount} encumbered pick_ownership assets, all have valid swap backing`
  );
  console.log('=== End Invariant Validation ===\n');

  // Sort assets by Year -> Holder -> ID
  allAssets.sort((a, b) => {
    if (a.seasonYear !== b.seasonYear) return a.seasonYear - b.seasonYear;
    if (a.holderTeam !== b.holderTeam)
      return a.holderTeam.localeCompare(b.holderTeam);
