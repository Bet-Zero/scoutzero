...
**R5 maker complete; independent R5 checker pending.** This receipt records
only the bounded maker construction for C14–C25 on
`architect/cba-canon-v2`. It does not accept R5, start the checker, or
unblock/start R6. Phase 2, Architect comparison, application work, Linear,
`main`, and prior accepted records remain untouched.
...
VIII, X, XI, XII, XIII, XXV, and XXIX. No secondary source certifies any
substantive rule, and no other primary artifact was needed for this scope.

## Coverage

R5 adds 12 GROUPs, 250 active LEAFs, 250 evidence components
(`EV2-0280`–`EV2-0529`), 78 new historical edges
...
|---|---|---:|
| C14 | Bird clocks/status, renunciation, first-year authority, term/raises, one-year consent and rights consequences | 24 |
| C15 | Arenas Offer Sheet shape, Room preservation, and offering/matching Team Salary treatment | 12 |
| C16 | Rookie Scale, maximum/Higher Max, award thresholds, Rookie Scale and Veteran/Designated Veteran Extensions, undrafted rookie status | 37 |
| C17 | Over-38 triggers, age dates, zero Seasons, QVFA exception, allocation, recurring reattribution | 9 |
| C18 | ordinary, trade-earned, and every Extension-bonus timing/allocation branch | 13 |
| C19 | Ten-Day opening, duration, capacity, hardship, termination, successor and Rest-of-Season term/proration | 20 |
| C20 | Two-Way roster, compensation, eligibility, March 4, conversion, games, advances, protection, affiliate and rights rules | 31 |
| C21 | Exhibit 9/10 shape, limits, waiver/designation/reporting, payment exceptions, conversion, Team Salary, notice, and Summer Contract timing | 33 |
| C22 | protection progression, Minimum bonus exceptions, July 1 maximum adjustment order, annual changes | 14 |
| C23 | incentive caps/provisos, signing bonuses, deferral, international-player payments, loans, insurance premiums | 27 |
| C24 | Team/Player Options and ETO shape, protection, deadlines, notices, extension interactions | 17 |
| C25 | retired, pending, circumvention, and grievance Team Salary treatment and reconciliation | 13 |

No substantive C14–C25 rule remains unsupported, ambiguous, or unavailable.
Three published rows were not legal rules: C17.7 was an input checklist, and
C19.6/C20.9 were missing-owner notes. They are preserved as exact
`process-only` dispositions; the actual January 5 and March 4 rules have
direct current owners. No `unsupported-residual`, OPS, or unavailable-source
claim was created.
...
The R5 maker changed only the canon, governing plan, and this concise receipt.
It introduced no schema, taxonomy, ID grammar, validator, registry model,
proof/lineage system, bundle, search-manifest, or infrastructure change.

- Targeted invocation of the frozen document-tree controls — PASS for every R5
  population and changed reference: 37 GROUPs, 521 total LEAF-main/detail rows
  (250 from R5), 351 XW2 edges, 529 EV2 components, 123 DR2 records, and 341
  fragments. The only seven diagnostics are the post-R4 simplified-plan versus
  stale-validator wording mismatches already present at starting checkpoint
  `f77d167e2f849ac1c4f33de3494252d4999ba46c`; no R5-local diagnostic remains.
- `python3
  work/architect-completion/cba_canon_v2_foundation_validator.py` — the single
  ...
  The command exits 1 only because the same seven pre-existing plan-wording
  diagnostics make its legacy `baseline_clean` summary false.
- Touched-file Markdown lint — the plan and this receipt pass. The canon reports
  only its 74 preserved MD029 findings in the unchanged numbered scenario
  blocks; the identical 74 findings reproduce against the pre-R5 canon from
...
- Scope confirmation — exactly the canon, governing plan, and this receipt are
  ...
  data, graph-output, Linear, `main`, R5-checker, or R6 work occurred.

The independent R5 checker and R6 remain unstarted.
