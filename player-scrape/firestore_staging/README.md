...
The correct workflow for pushing player data to Firestore is:

1. **Scrape** - Run contract scraper: `npx tsx player-scrape/contracts/scripts/run_contracts.ts`
2. **Stage** - Stage all players: `npx tsx player-scrape/firestore_staging/scripts/stage_all_players.ts`
3. **Validate** - Validate staged files (automatic in push script, or run manually):
   ```bash
   npx tsx player-scrape/firestore_staging/scripts/validate_staged.ts [playerId...]
   ```
4. **Push** - Push to Firestore (validation runs automatically): `npx tsx player-scrape/firestore_staging/scripts/push_staged_players.ts player_id [player_id...]`

**Important**: The push script automatically validates all players before pushing. If validation fails, the push is aborted with an error message. This ensures only valid data is written to Firestore.
...
All staged files are validated against Zod schemas before pushing to Firestore. The validation checks:

- Main player document structure (`players_v2/{playerId}`)
- Contract subcollection documents (`players_v2/{playerId}/contracts/{contractId}`)
- Season subcollection documents (`players_v2/{playerId}/seasons/{seasonId}`)
- Contracts view document (`players_v2/{playerId}/views/contracts`)
- BasePlayer document (`architect/basePlayers/{playerId}`)

### Manual Validation

You can validate staged files manually before pushing:

```bash
# Validate specific players
npx tsx player-scrape/firestore_staging/scripts/validate_staged.ts luka_doncic kevin_durant

# Validate all staged players
npx tsx player-scrape/firestore_staging/scripts/validate_staged.ts
```

The validation script will report any schema violations and exit with code 1 if any players fail validation.

### Automatic Validation

The push script (`push_staged_players.ts`) automatically validates each player before pushing. If validation fails for any player, the push is aborted and no data is written to Firestore.
...
```bash
npx tsx player-scrape/firestore_staging/_dev/dry_run_write.ts --players=toumani_camara,luka_doncic
```
...
├── README.md                    # This file
├── scripts/
│   ├── stage_player.ts          # Core staging logic
│   ├── stage_all_players.ts     # Batch staging CLI
│   ├── push_staged_players.ts   # Firestore write CLI
│   ├── validate_staged.ts       # Validation CLI
│   └── run_full_scrape.ts       # Full pipeline runner
├── _dev/
│   └── dry_run_write.ts         # Preview writes without executing
├── _artifacts/
│   └── output/                  # Staged JSON files (git-ignored)
└── docs/                        # Structure documentation
    ├── players_v2_structure.md
    └── basePlayers_structure.md
```
...
- `player-scrape/firestore_staging/_artifacts/output/players_v2/` (generated, git-ignored)
- `player-scrape/firestore_staging/_artifacts/output/basePlayers/` (generated, git-ignored)
