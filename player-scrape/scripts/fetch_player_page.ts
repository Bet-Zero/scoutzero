...
//   Saves the fully rendered HTML to examples/page.html for parsing with parse_player.ts
//
// RUN:
...
try {
  ...
  // Save to file
  const outputPath = join(__dirname, '../examples/page.html');
  ...
  if (
    ...
    html.includes('Salary')
  ) {
    console.log('✅ Salary table detected in HTML');
  } else {
    console.warn(
      '⚠️  Warning: Salary table may not be present in rendered HTML'
    );
  }
} catch (error: any) {
