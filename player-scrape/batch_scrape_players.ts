...
//
// RUN:
//   PLAYERS_FILE="players_list.json" OUTPUT_DIR="output/players" npx tsx player-scrape/batch_scrape_players.ts
//
...
const config: ScraperConfig = {
  playersFile: process.env.PLAYERS_FILE || 'players_list.json',
  outputDir: process.env.OUTPUT_DIR || 'output/players',
...
async function parsePlayerData(html: string, playerId: string, teamCode: string, playerUrl: string) {
  ...
  // Extract player name
  const displayName = norm($('h1').first().text()) || playerId.replace(/_/g, ' ');
  ...
  const isRookieScale = contractTypeText.includes('ROOKIE') || contractTypeText.includes('SCALE');
  const isDesignated = contractTypeText.includes('DESIGNATED');
  const isTwoWay = contractTypeText.includes('TWO-WAY') || contractTypeText.includes('2-WAY');
  
  let contractType = 'VETERAN CONTRACT';
  if (isTwoWay) contractType = 'TWO-WAY';
  else if (isDesignated && isRookieScale) contractType = 'DESIGNATED ROOKIE EXTENSION';
  ...
  const restrictedMatch = teamText.match(/Cannot be traded until[:\s]+(\d{1,2}\/\d{1,2}\/\d{4})/i);
  ...
  const baseYearCompensation = teamText.toLowerCase().includes('base year compensation') || teamText.toLowerCase().includes('byc');
  const poisonPill = isRookieScale && teamText.toLowerCase().includes('poison pill');
...
async function batchScrape() {
  ...
  // Load players list
  const playersListPath = join(process.cwd(), config.playersFile);
  if (!existsSync(playersListPath)) {
    console.error(`❌ Players file not found: ${playersListPath}`);
    console.error('Create a JSON file with format: [{ "playerId": "...", "slug": "...", "teamCode": "..." }]');
    process.exit(1);
  }
  
  const playersList: PlayerInput[] = JSON.parse(await fs.readFile(playersListPath, 'utf-8'));
  console.log(`📋 Loaded ${playersList.length} players from ${config.playersFile}`);
