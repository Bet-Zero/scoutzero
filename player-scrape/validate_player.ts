...
//
// RUN:
//   npx tsx player-scrape/validate_player.ts
//
// USAGE:
...
import { basePlayerSchema } from './player_scrape_schema.js';

const __dirname = dirname(fileURLToPath(import.meta.url));

async function validatePlayer() {
  const playerFile = process.env.PLAYER_FILE || 'player.json';
  const filePath = join(__dirname, playerFile);
  ...
  try {
    ...
    console.log(`   Rookie Scale: ${data.contract.isRookieScale ? 'Yes' : 'No'}`);
    console.log(`   Years: ${data.contract.contractLength} (${data.contract.startSeason} - ${data.contract.endSeason})`);
    console.log(`   Total Value: $${(data.contract.totalValue / 1000000).toFixed(1)}M`);
    console.log(`   Bird Rights: ${data.contract.birdRights.status}`);
    console.log(`   Trade Eligible: ${data.contract.tradeEligibility.canBeTradedNow ? 'Yes' : 'No'}`);
    if (!data.contract.tradeEligibility.canBeTradedNow) {
      console.log(`   Restricted Until: ${data.contract.tradeEligibility.restrictedUntil}`);
