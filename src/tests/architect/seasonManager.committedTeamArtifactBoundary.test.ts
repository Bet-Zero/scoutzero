...
vi.mock('@/features/architect/utils/capTotals', () => ({
  ...
  createCanonicalTeamTotalsSnapshot: (team: UnknownRecord, year: number) => ({
...
vi.mock(
  ...
  async (importOriginal) => {
    const actual =
      ...
      >();
    const snapshot = (team: UnknownRecord, year: number) => ({
      ...mocks.computeTeamCapTotals(team, year),
      teamSalary: 20_000_000,
      apronTeamSalary: 21_000_000,
      taxSalary: 22_000_000,
    ...
    return {
      ...
      synchronizeTeamTotalsSnapshot: (team: UnknownRecord, year: number) => ({
        ...team,
