...
describe('seasonManager raw draft-pick ingress closure', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('normalizes mixed raw snapshot ingress before the committed season-advance write path', async () => {
  ...
  it('tolerates projected/raw conveyance markers only at the outer ingress boundary', () => {
        ...
        draftPicks: [
          {
            ...
            tradeable: true,
            via: 'MEM',
            _sourceEntitlementId: 'ent-1',
            _projectedAt: '2026-04-01T00:00:00.000Z',
          },
        ],
      },
      2026,
    ...
    expect(pick?.conveyance?.conditions.protection).toBe('Unprotected');
    expect(pick?.conveyanceResult).toEqual(
      expect.objectContaining({
        outcome: 'rolled',
        position: 5,
        previousYear: 2026,
        previousProtection: 'Lottery protected',
      })
    );
    expect(pick).not.toHaveProperty('tradeable');
    expect(pick).not.toHaveProperty('via');
    expect(pick).not.toHaveProperty('_sourceEntitlementId');
    expect(pick).not.toHaveProperty('_projectedAt');
  });

  it('still resolves swaps on the normalized draft-pick carrier contract', () => {
