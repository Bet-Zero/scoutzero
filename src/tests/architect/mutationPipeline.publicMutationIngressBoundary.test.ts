...
describe('mutationPipeline public mutation ingress boundary', () => {
  it('hands loader state to sign-free-agent compute as a narrowed lane and preserves commit/reload continuity', async () => {
    ...
    expect(committedTeam).not.toHaveProperty('legacyTeamIngressBlob');
  ...
  it('runs a different mutation family after the central ingress adapter narrows to team-only state', () => {
  ...
  it('tolerates mixed direct-call compatibility only at the public compute boundary', () => {
    ...
    const result = computeWorldMutation({
      ...
      currentState: {
        ...
        offerSheetId: 'ignored_offer_sheet',
