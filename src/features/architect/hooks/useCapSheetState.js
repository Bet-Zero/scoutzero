...
export function useCapSheetState({ initialCapSheet, currentYear }) {
  ...
  const [, setCapHolds] = useState(() => initialCapSheet?.capHolds || []);
  ...
  const updatePlayer = useCallback(
    (playerIdOrName, updateFn, actionType, actionDetails = {}) => {
  ...
  const signPlayer = useCallback(
    (player, contractData, signingType = 'resign') => {
      const playerId = player.playerId || player.id || player.name;
      ...
      updatePlayer(
        ...
        (p) => {
          ...
          const years = contractData.years || providedSalaries.length || 1;
          const baseSalary =
   ...
   */
  const renounceRights = useCallback(
    (playerOrHold) => {
      const idToRenounce =
        playerOrHold.playerId ||
        playerOrHold.id ||
        playerOrHold.player_id ||
        playerOrHold.name;
