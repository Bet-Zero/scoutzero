...
export function useTeamSnapshot(teamId, result) {
  ...
  return {
    ...
    outgoingBaseSalary: teamResult.calculations?.salaryOut ?? teamResult.salaryOut ?? 0,
    ...
    incomingBaseSalary: teamResult.calculations?.salaryIn ?? teamResult.salaryIn ?? 0,
    ...
    allowableIncoming: teamResult.rules?.salaryMatching?.allowableIncoming 
    ...
    salaryMatchingApplicable: teamResult.rules?.salaryMatching?.applicable ?? true,
    salaryMatchingSkipReason: teamResult.rules?.salaryMatching?.skipReason ?? null,
    
    salaryMatchingRule: teamResult.rules?.salaryMatching?.details?.ruleApplied ?? 'unknown',
    salaryMatchingFormula: teamResult.rules?.salaryMatching?.details?.formulaUsed ?? '',
    margin: teamResult.rules?.salaryMatching?.margin 
    ...
    hardCapTriggered: teamResult.rules?.hardCap?.triggered ?? teamResult.hardCapped ?? false,
 ...
 * @returns {Object|null} Team snapshot with legality-affecting values, or null if not available
 */
export function useTeamSnapshotByIndex(teamIndex, result) {
  ...
  return useTeamSnapshot(teamResult.teamId, result);
}
...
export function useTradeSnapshot(result) {
  ...
  return {
    ...
    capSettings: result.capSettings ?? result.tradeReceipt?.capSettingsUsed ?? null,
    
    // All violations across all teams
    allViolations: result.teamResults?.flatMap(tr => tr.violations ?? []) ?? [],
