...
type RFAContractLike = {
  isRookieScale?: boolean | null;
  contractType?: string | null;
  yearsRemaining?: number | null;
  endSeason?: string | null;
  startSeason?: string | null;
  freeAgency?: ContractFreeAgencyLike | null;
  salariesByYear?: PlayerSalaryEntryLike[] | null;
...
type RFAPlayerLike = {
  ...
  contract?: RFAContractLike | null;
};

type RFALeagueContextLike = {
 ...
 */
export function computeQualifyingOffer(
  player: RFAPlayerLike | null | undefined,
  leagueContext?: RFALeagueContextLike | null
): QualifyingOfferInfo {
  ...
  const minimumSalary =
    minSalaryScale[Math.min(yearsOfService, 10)] || minSalaryScale[0];
 ...
 */
function isContractExpiring(
  contract: RFAContractLike | null | undefined,
  currentYear: number
): boolean {
 ...
 */
function getDraftYear(player: RFAPlayerLike | null | undefined): number | null {
  const draftYear = player?.bio?.draftYear || player?.draftYear;
  if (!draftYear) return null;

  const year = parseInt(String(draftYear), 10);
...
function getDraftPosition(
  ...
  if (pick) {
    const pickNum = parseInt(String(pick), 10);
 ...
 */
function getRookieQOIncrease(draftPosition: number | null): number {
...
function getPriorSalary(
  ...
  let salaryEntry = contract.salariesByYear.find(
    (entry) => entry.season === currentSeason
  );

  if (!salaryEntry) {
    // Try prior season
    salaryEntry = contract.salariesByYear.find(
      (entry) => entry.season === priorSeason
    );
  }
  ...
  if (!salaryEntry && contract.salariesByYear.length > 0) {
    ...
    const sorted = [...contract.salariesByYear].sort((a, b) => {
      const yearA = parseSeasonEndYear(a.season || undefined) || 0;
      const yearB = parseSeasonEndYear(b.season || undefined) || 0;
      return yearB - yearA;
    });
 ...
 */
function getContractSigningYear(
  contract: RFAContractLike | null | undefined
): number | null {
 ...
 */
function computeQOAcceptanceDeadline(currentYear: number): Date {
...
export function computeRFAFromRuleContext(
  ...
  if (!playerCtx.contractEndSeasonId) {
    return {
      ...
      qualifyingOfferEligible: false,
      reason:
        'Cannot compute RFA status: missing contract expiry validation (no contractEndSeasonId)',
      status: RFA_STATUS.UNKNOWN,
    };
  }

  // Build a synthetic player object from RuleContext
  const syntheticPlayer: RFAPlayerLike = {
