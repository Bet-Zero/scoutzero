 ...
 *
 * @file src/features/architect/utils/tpeLifecycle.ts
 */

import type { TradeExceptionRecord } from './tradeMachine/constants/types';
import { toEndYear } from './seasonFormat';

type TpeExpiryParams = {
  expiryStr: string;
  boundaryStr: string;
};

export type TpeLifecycleRecord = TradeExceptionRecord & {
  expiresOn?: string | null;
  expiryISO?: string | null;
  expiryDate?: string | null;
  _expiryParams?: TpeExpiryParams;
};

export interface ProcessTradeExceptionsResult {
  hasChanges: boolean;
  activeTPEs: TpeLifecycleRecord[];
  expiredTPEs: TpeLifecycleRecord[];
}

// Helper to get canonical expiry date string
export function getTpeExpiryISO(
  tpe: TpeLifecycleRecord | null | undefined
): string | null {
  if (!tpe) return null;
 ...
 *
 * @param tradeExceptions - List of TPE objects from team.tradeExceptions
 * @param toSeason - Target season code (e.g. "2026-27")
 * @returns Result { hasChanges, activeTPEs, expiredTPEs }
 */
export function processTradeExceptions(
  tradeExceptions: TpeLifecycleRecord[] | null | undefined,
  toSeason: string
): ProcessTradeExceptionsResult {
  // Defensive check
  ...
  const activeTPEs: TpeLifecycleRecord[] = [];
  const expiredTPEs: TpeLifecycleRecord[] = [];
