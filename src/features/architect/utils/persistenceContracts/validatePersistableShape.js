/**
 * FILE: src/features/architect/utils/persistenceContracts/validatePersistableShape.js
 ...
 * @param {string[]} params.allowlist - Array of allowed key names (top-level)
 * @param {string} [params.pathPrefix=''] - Prefix for path reporting (e.g., 'team')
 * @param {Object} [params.deepRules=null] - Map of nested paths to their allowlists
 *   Example: { 'exceptions.tpe': [...], 'exceptionHistory': [...], 'deadCap.amountByYear': [...] }
 *   Supports 3-level nesting for paths like 'parentArray.nestedArray'
 * @param {number} [params.maxDepth=10] - Max recursion depth (safety limit)
 * @returns {string[]} Array of disallowed key paths (e.g., ['team.foo', 'team.tradeExceptions[0].debug'])
 */
        ...
        value.forEach((item, index) => {
          if (item && typeof item === 'object') {
            ...
            const itemViolations = findDisallowedKeyPaths({
              ...
              deepRules: itemDeepRules, // Pass nested rules for 3-level validation
              maxDepth: maxDepth - 1,
            });
 ...
 * @param {Object} params - Validation parameters
 * @param {any} params.obj - Object to validate
 * @param {string[]} params.allowlist - Array of allowed key names
 * @param {string} params.label - Human-readable label (e.g., 'TEAM', 'PLAYER')
 * @param {Object} [params.deepRules=null] - Map of nested paths to their allowlists
 * @returns {{ valid: boolean, violations: string[], label: string }}
 */
 ...
 *
 * @param {string[]} violations - Array of violation paths
 * @param {string} label - Document type label
 * @param {number} [maxShow=25] - Maximum violations to show
 * @returns {string} Formatted error message
 */
export function formatViolationMessage(violations, label, maxShow = 25) {
