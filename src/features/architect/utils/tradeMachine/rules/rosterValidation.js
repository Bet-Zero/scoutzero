...
export function validateRosterWindow(team, ctx = {}) {
  ...
  // Method 2: Use projectedRosterCount if available
    ...
    standardRosterSize = team.projectedRosterCount;
    twoWayRosterSize = (team.incomingPlayers || []).filter((p) => p.contractType === 'two-way').length -
  ...
  // Method 3: Calculate from current roster + trades
    ...
    const incomingStandard = (incomingPlayers.filter((p) => !p.isTwoWay).length) || 
                            (receives.filter((p) => !p.isTwoWay).length);
    const outgoingStandard = (outgoingPlayers.filter((p) => !p.isTwoWay).length) || 
                            (sends.filter((p) => !p.isTwoWay).length);
    const incomingTwoWay = (incomingPlayers.filter((p) => p.isTwoWay).length) || 
                          (receives.filter((p) => p.isTwoWay).length);
    const outgoingTwoWay = (outgoingPlayers.filter((p) => p.isTwoWay).length) || 
                          (sends.filter((p) => p.isTwoWay).length);

    standardRosterSize = (team.standardContracts || team.team?.standardRoster?.length || 0) +
                        incomingStandard - outgoingStandard;
    twoWayRosterSize = (team.twoWayContracts || team.team?.twoWayRoster?.length || 0) +
                      incomingTwoWay - outgoingTwoWay;
  }
  ...
  if (twoWayRosterSize > MAX_TWO_WAY) {
    violations.push(
      `Two-way slots exceeded (${twoWayRosterSize}/${MAX_TWO_WAY})`
    );
  }
  ...
  return {
    ...
    violations,
    message: violations.length === 0 ? 'Roster size validated' : 'Roster size requirements not met',
 ...
 */
export function enforceRosterRules(team, ctx = {}, notifier = {}) {
...
export function enforceRosterWindowAdvanced(
  ...
  if (!isGracePeriod && (standardRosterSize < MIN_ROSTER || standardRosterSize > MAX_ROSTER)) {
    violations.push(
  ...
  if (isGracePeriod && standardRosterSize < GRACE_MIN_ROSTER) {
    violations.push(`Roster cannot go below ${GRACE_MIN_ROSTER} players during grace period`);
  }
