...
const RosterManager = ({
  ...
  const handleWaivePlayer = (player) => {
    const updatedRoster = teamCapSheet.players.filter(
          ...
          {teamCapSheet.players.map((player) => {
            ...
            return (
              <tr key={player.name} className="odd:bg-[#171717]">
                ...
                <td className="p-2">
                  {player.contract_clean?.yearsLeft || '—'}
                </td>
                <td className="p-2">
                  {player.contract_clean?.salaries_by_year?.[currentYear]
                    ?.salary
                ...
                <td className="p-2">
                  ...
                  ).some((y) => y.option === 'Player Option') && 'PO '}
                  {Object.values(
                    player.contract_clean?.salaries_by_year || {}
                  ).some((y) => y.option === 'Team Option') && 'TO '}
                </td>
