export function canSignFreeAgent(player, teamCapSheet, capSettings) {
  ...
  if (
    hardCapped &&
    player.askingSalary + teamSalary > capSettings.secondApron
  ) {
    return {
      allowed: false,
  ...
  if (
    rights === 'Early' &&
    player.askingSalary >
      Math.max(player.previousSalary * 1.75, capSettings.averageSalary * 1.05)
  ) {
    return {
      allowed: false,
...
export function generateContract(player, capSpace) {
  return {
    years: 2,
    amount: capSpace / 2,
  };
