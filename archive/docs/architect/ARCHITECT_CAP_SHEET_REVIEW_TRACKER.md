...
| CS-5D | Add Focused Guardrails for UI-to-Validation-to-Persistence Mutation Truth                                | DONE   | Focused CS-5D guardrails still pin the authoritative Cap Sheet stage order, preview-operationId persistence handoff, preview-event linkage, and in-place rollback semantics across the manual dead-money / exceptions path. On the March 29, 2026 live-workspace follow-up rerun, the required closeout gate (`npm run typecheck`, `npm run test:diff -- --reporter=dot`, `npm run build`) was green. Direct evidence reruns of `src/tests/architect/signAndTrade.test.js` and `tests/architect/renounceRights.test.js` remain red, but they are outside the required Step 5 closeout gate and do not change the Cap Sheet mutation-truth conclusion. |
...
| CS-6D | Add Focused Guardrails for Contract-Action Integration Truth                                      | DONE   | Focused dashboard behavior tests and closure gates now pin rules-year vs target-year threading, world-only preflight callback withholding, thin modal dispatch adapters, current-year Cap Sheet launch fencing, and the shared mutation-owner route. |
