...
2. Try current world snapshot at `/architect/worlds/{worldId}/snapshot/teams/{teamCode}`
3. If not found, recursively check parent world
4. Fall back to base

**getPlayer() function**:
1. If no worldId, read from `/architect/basePlayers/{playerId}`
2. Try world-specific player override at `/architect/worlds/{worldId}/snapshot/teams/{teamCode}/players/{playerId}`
