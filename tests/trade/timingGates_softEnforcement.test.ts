...
type TimingTeam = Parameters<typeof enforceTiming>[0];
type TimingContext = Parameters<typeof enforceTiming>[1];
type TimingResult = ReturnType<typeof enforceTiming>;

const getIssues = (
  issues: TimingResult['warnings'] | TimingResult['violations']
) => issues ?? [];

const run = (team: TimingTeam, tradeCtx: TimingContext) => {
  const warns: string[] = [];
...
describe('timing gates soft enforcement', () => {
  ...
  it('moratorium rule respects flag', () => {
    ...
    expect(getIssues(r.result.violations)).toHaveLength(1);
    expect(getIssues(r.result.warnings)).toHaveLength(0);
    validationFlags.timingEnforcement = 'warn';
    r = run(team, tradeCtx);
    expect(r.warns).toHaveLength(1);
    expect(r.warns[0]).toMatch(/moratorium/i);
    expect(r.result.passed).toBe(true);
    expect(getIssues(r.result.violations)).toHaveLength(0);
    expect(getIssues(r.result.warnings)).toHaveLength(1);
  });
  ...
  it('does not surface the retired 2-month aggregation rule', () => {
    ...
    expect(
      getIssues(r.result.violations).some((issue) =>
    ...
    expect(
      getIssues(r.result.warnings).some((issue) =>
        /acquired within the last 60 days/i.test(issue.message)
      )
    ).toBe(false);
  });
