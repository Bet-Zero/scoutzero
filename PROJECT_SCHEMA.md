...
| `architect-plan-summary/` | Architecture planning documents |
...
- `architect-teams-plan/` - Teams collection migration plans
...
assert(filename === content.playerId, 
...
**Validation:**
```typescript
assert(/^[a-z]+(_[a-z]+)*$/.test(playerId),
...
assert(contract.guaranteedYears === guaranteedYears,
  'guaranteedYears mismatch');
assert(contract.guaranteedValue === guaranteedValue,
