...
- A signing bonus is allocated over the Salary Cap Years covered by the contract **in proportion to the percentage of Base Compensation in each such Salary Cap Year that, at the time of allocation, is protected for lack of skill** — not in proportion to guaranteed salary dollars. If the contract provides for an ETO, allocation runs only over the Salary Cap Years preceding the ETO's effective season. If no Base Compensation is protected for lack of skill at the time of allocation, the entire bonus is allocated to the first Salary Cap Year (for a bonus earned on trade, the cap year of the trade) (CBA VII §3(b)(2), pp. 200–201).
...
- Incentive Compensation for a season may not exceed 20% of the **Regular Salary** called for by the contract for that season (CBA II §12(a)(i), p. 58). Unlikely Bonuses in a Salary Cap Year may not exceed 15% of the player's **Regular Salary** for that Salary Cap Year at the time the contract is signed, subject to the CBA's extension/renegotiation provisos (CBA VII §5(b)(1), p. 229). Both caps use the defined Regular Salary denominator, not Base Compensation; the two defined amounts can differ.
...
- **Player payment:** if a team's MTS Payment Team Salary is below the Minimum Team Salary, the team pays the NBA the difference between MTS Payment Team Salary and the Minimum Team Salary, and is barred from sharing in any tax distribution to non-taxpayers (VII §2(c)(2)).
...
| CBA-C10.3 | CBA-C10 | §8.7 | If MTS Payment Team Salary is below the Minimum Team Salary, the team pays the difference computed on that base and loses the non-taxpayer tax distribution; the in-season Team Salary charge is separately the shortfall of the lesser of then-current and season-start MTS Cap Hold Team Salary — different adjusted bases, not necessarily equal amounts | CBA VII §2(c)(1)–(3) pp. 176–178 | SCEN | #60 |
...
| CBA-C18 | *(none — top-level leaf)* | §5.9 | A signing bonus is allocated over the covered Salary Cap Years in proportion to the percentage of Base Compensation in each year that is protected for lack of skill at allocation time; with an ETO, allocation runs only over years preceding the ETO's effective season; if no Base Compensation is protected for lack of skill, the whole bonus is allocated to the first Salary Cap Year | CBA VII §3(b)(2) pp. 200–201 | SCEN | #67 |
...
| CBA-C23.4 | CBA-C23 | §5.9 | Incentive Compensation for a season <=20% of Regular Salary; Unlikely Bonuses in a Salary Cap Year <=15% of Regular Salary at signing, subject to the extension/renegotiation provisos | CBA II §12(a)(i) p. 58; VII §5(b)(1) p. 229 | SCEN | #68 |
...
50. Re-trade of an exception-acquired player: aggregated versus solo. **Input:** a player acquired via an exception on November 20; case (a) the team re-trades him alone (unaggregated) on January 10; case (b) the team includes him among two or more aggregated Traded Players on January 10; case (c) the same player, acquired on or before December 16, is aggregated in a trade on the day before the trade deadline and again on the deadline itself. **Boundary:** the two-month restriction bars only his inclusion among the Traded Players being **aggregated** under CBA VII §6(j)(1)(ii)–(iv); it does not bar a solo re-trade; the December 16 carve-out lifts the bar for trades on or after the day prior to the deadline. **Expected:** (a) legal despite being inside two months; (b) blocked until the two months elapse; (c) legal under the carve-out. **Authority:** CBA VII §6(j)(4)(i), p. 242.
...
60. Minimum team salary: the payment and the Team Salary charge are two different calculations. **Input:** a team below the Minimum Team Salary at the start of the regular season whose MTS Payment Team Salary differs from its MTS Cap Hold Team Salary (it carries salary excluded from Team Salary under VII §4(h) or §4(b) and salary included under VII §3(e)), and whose then-current MTS Cap Hold Team Salary later drops below its season-start value. **Boundary:** the player payment equals the Minimum Team Salary minus **MTS Payment Team Salary**; the in-season Team Salary charge equals the Minimum Team Salary minus the **lesser of then-current and season-start MTS Cap Hold Team Salary**; the two bases carry different adjustments. **Expected:** the payment and the Team Salary charge are computed separately and differ on these facts; the team also loses the non-taxpayer tax distribution, and the year-end §2(c)(5) reconciliation re-tests with excluded-but-earned incentives added and included-but-unearned incentives removed. **Authority:** CBA VII §2(c)(1)–(3) and (5), pp. 176–178.
...
67. Signing-bonus allocation on the protected-percentage basis. **Input:** a multi-year contract with a signing bonus whose Base Compensation protection for lack of skill differs by season (e.g., 100% / 50% / 0%); a variant whose contract contains an ETO; a variant with no lack-of-skill protection in any season. **Boundary:** allocation is in proportion to **the percentage of Base Compensation in each Salary Cap Year that is protected for lack of skill at allocation time** — not in proportion to guaranteed salary dollars; the ETO variant allocates only over Salary Cap Years preceding the ETO's effective season; the zero-protection variant allocates the entire bonus to the first Salary Cap Year. **Expected:** each variant's per-year allocation matches the protected-percentage proportions, and a dollar-proportional allocation is detected as wrong whenever the two bases diverge. **Authority:** CBA VII §3(b)(2), pp. 200–201.
...
69. Options and ETOs at the correct boundaries. **Input:** a standard option exercised at 5:00 p.m. ET on June 29; an RFA-triggering option exercised on June 25; a five-season contract with an ETO taking effect at the end of the fourth season (eliminating the fifth); a contract with an ETO purporting to take effect **before** the end of the fourth season; and an ETO exercised before an attempted extension. **Boundary:** an ETO may take effect no earlier than the end of the fourth season of the contract — the earliest season it can eliminate is the fifth; an RFA-triggering option must be exercised prior to June 25, so June 25 is late. **Expected:** the June 29 exercise is legal; the June 25 RFA-option exercise fails; the ETO effective at the end of the fourth season (a legal ETO that leaves the first four seasons intact) passes; the ETO effective before the end of the fourth season fails; the post-ETO extension attempt fails. **Authority:** CBA XII §2(b), p. 337; XII §4, p. 338.
