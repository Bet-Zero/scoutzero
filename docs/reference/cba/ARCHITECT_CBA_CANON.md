...
- The percentage/cap basis is the Base Compensation remaining to be earned under the Contract at the time of the trade — the unearned current-season remainder plus future-season Base Compensation, with no guarantee/protection filter — excluding an Option Year not yet exercised (CBA XXIV §2(a)(ii)–(iii)(A), pp. 414–15). *Corrected by R3: the previous "guaranteed base compensation still owed" basis is not the signed rule.*
- Allocate the result as a VII §3(b)(1)(ii) signing bonus over the then-current and any remaining Salary Cap Years in proportion to each year's lack-of-skill-protected percentage of Base Compensation (VII §3(b)(2), pp. 200–201). For a Rookie Scale Contract, the trade bonus is deemed amended to the extent necessary to keep Salary plus Unlikely Bonuses at or below 120% of the applicable Rookie Scale Amount (VIII §1(d), p. 293). *Corrected by R3: no general veteran maximum-salary reduction of a trade bonus is expressed in the signed text — Article II §7 caps amounts at signing, renegotiation, and extension only.*
...
### 15.9 Register and source-certification standard (v2.0 — binding, R2.1 edition as hardened by R2.2 and corrected by R2.3, R2.4, R2.5, R2.6, R2.7, and R2.8)
...
(executed; rejected) → R2.8 → independent Codex R2.8 review → R3.1 →
...
- **Fragment-inventory schema (pinned; one row per fragment, in the
  ...
  record (below) for a fragment carried by a nonterminal bundle, or `—`
  for a fragment carried by exactly one terminal edge; `Fragment status`
...
- **Normalized fragment scope (pinned representation and normalization
  algorithm — R2.8).** The scope is a `"; "`-separated, ascending,
  pairwise non-overlapping list of **scope atoms** over the published
  v1.1 LEAF's Canonical requirement text (meaning fixed at `9814939c`,
  §15.9.1). Each atom is either a **source-clause coordinate**
  `clause:<locator>` — the signed text's own enumeration
  (article/section/subsection/clause, e.g., `clause:VII§6(j)(4)(i)`),
  used where the LEAF text is itself enumerated — or a **sentence-span
  coordinate** `sent:<a>` or `sent:<a>-<b>` — 1-indexed sentence
  ordinals (`a ≤ b`) within that LEAF's published Canonical requirement
  text, used for prose. Normalization: order atoms ascending; a
  `sent:<a>-<b>` span denotes the contiguous ordinals `a..b`; two atoms
  overlap iff they share a clause coordinate or their sentence-ordinal
  ranges intersect. **Scope equality** is set equality of normalized
  atoms. **Pairwise non-overlap** across a LEAF's fragments is disjoint
  atom sets. **Exhaustive coverage** (declared under the decomposition
  decision record) is that the union of the LEAF's fragment atoms covers
  every sentence ordinal `1..S` and every enumerated clause of the
  LEAF's Canonical requirement text. **Textual contiguity**, where a
  fragment declares itself a contiguous unit, requires its sentence-span
  atoms to form one contiguous ordinal range. Fragment IDs are
  contiguous `F1…Fn` with no gaps at declaration; a fragment's scope
  atoms are its **stable identity** across `AMEND` history — successor
  fragments re-cover a superseded fragment's atoms and no atom is
  silently dropped.
- **Disposition-bundle schema (pinned; fixed; parseable — R2.8).** A
  fragment that maps to multiple compatible active targets does so
  through **one** governed bundle, not prose. Bundles are a support
  ...
  members**, ≥1 `XW2-…` ID. `Member edge types` and `Member target IDs`
  are aligned position-for-position with `Member edge IDs`; every member
  edge type is drawn only from the **nonterminal** vocabulary
  (`equivalent`, `split`, `merge`, `partial-overlap`, `moved` — never a
  terminal type, never an unknown type), and **no duplicate (source,
  target) mapping** appears. `Subject scope` equals the fragment's
  Normalized fragment scope. `Bundle class` is `active` (a bundle is
  always the nonterminal, actively owned disposition; a terminal
  fragment carries a single terminal edge, never a bundle —
  **active/terminal mixing fails**). `Bundle status`/`Bundle version`
  are `current` | `superseded` / integer. `Superseding/current
  relationship` is `—` or `supersedes BND-<NNNN> per AMEND DR2-<NNNN>`.
  A bundle whose member edges do not all point back to its
  `Source fragment ID`, that mixes a terminal member, that carries a
  duplicate edge or duplicate source-target mapping, or that contradicts
  the fragment's single disposition, fails. The SXW2 analog (§15.9.8)
  uses `SXW2-…` member edges over a scenario fragment. R2.8 mints no
...
- **Fragment kinds (closed vocabulary; exactly one per fragment):**
  ...
  `authority-assertion` (a claim that some authority class makes a
...
- **Per-LEAF completeness contract (all mandatory):** (1) a declared
  **exhaustive decomposition** — the inventory states that the listed
  ...
  fragment **dispositioned exactly once**: either by exactly one
  terminal edge (with `Disposition bundle ID` = `—`), or by exactly one
  **governed disposition bundle** (`BND-<NNNN>`, above) whose member
  nonterminal edges together carry the fragment under
  split/merge/`partial-overlap` semantics and equal the fragment's
  `Disposition edge ID(s)`. One fragment may legitimately map to
  multiple active targets through its one bundle; it may never carry two
  terminal edges, never a terminal edge plus a bundle, and never two
  bundles (no fragment is simultaneously terminal and actively owned,
  and no fragment carries contradictory dispositions); (5)
...
class` is `XW2-DISP` | `SXW2-DISP`; `Subject historical LEAF` is the
published v1.1 LEAF (`—` for a scenario subject) and `Subject fragment
or candidate` the fragment ID and preserved candidate; `Finding type` is
the closed vocabulary `blocked-unsupported-obligation`;
`Search-set/manifest references` names the current `SS2-…` set and its
`SM2-…` members proving the adequate, not-located search;
`Preserved candidate anchor` is the canon anchor; `Finding status` is a
closed vocabulary `open` | `resolved` (an `open` finding fails U7 and
stops the unit); `Resolution ID` names the current `RES-…` resolution
once one exists, else `—`.
...
§15.9.6 verifier grammar (`agent:<slug>`/`human:<slug>`);
`Independent acceptance commit/receipt` is the pinned acceptance commit
plus receipt path, or `—` while unaccepted; `Resolution status` is a
closed vocabulary `proposed` | `accepted` | `superseded`;
`Resolution version` is an integer from `1`.

**Independent-acceptance gate (binding — R2.8).**

1. A resolution is `accepted` **only** when `Independent checker
   identity` differs from `Maker/proposer identity` and a pinned
   `Independent acceptance commit/receipt` is recorded — **a maker can
   never self-accept or clear its own blocked finding**.
2. A `BLK-…` finding becomes `resolved` **only** when its `Resolution
   ID` names a current `accepted` resolution; otherwise it stays `open`,
...
6. `BLK-…`/`RES-…` records are an `AMEND`-governed live population:
   open/current/resolved status, direct current references, and one
   current accepted resolution per chain are enforced by U7, `G15R`,
   G15, and R9 (§15.9.9).
...
**Subject-family exclusivity, equality, and grouping (binding — R2.8).**
Mechanical equality and bidirectional reconciliation are checked across
the full tuple **(subject class; historical LEAF or scenario; fragment;
normalized scope; no-owner-reason basis; terminal edge; terminal edge
type; generic `DR2-…` parent; `DISP` detail row; current
status/version)** — the edge and the detail row agree on every element,
in both directions. Uniqueness/grouping keys: an `XW2-DISP` terminal
disposition is unique per **(historical source LEAF, historical fragment
ID)**; an `SXW2-DISP` terminal disposition is unique per **(historical
scenario, scenario fragment ID)**; and at most one current `DISP` detail
row exists per terminal-subject key **and** basis (no-owner reason). One
`DISP` record may cover multiple detail rows only where those rows share
**one** subject class **and** their terminal bases are demonstrably
identical, with every covered edge ID listed; an `XW2-DISP` record never
carries an `SXW2-DISP` row and vice versa (**subject-family mismatch is
...
terminal bases are demonstrably identical under the detail schema
above**; `DR2-0039`'s `ATOM` (atomicity) content is preserved only
...
`Record ID | Provenance type | Source/provenance identity | Source date (basis:value) or — | Official URL or — | Artifact SHA-256 or — | Retrieval timestamp or — | Authentication timestamp or — | Verifier identity | Verification session ID | Verification date | Record limitations | Record status/version`
...
- **Field grammar.** The base `Source date` field is exactly
  ...
  is malformed. The pair states one **primary** semantic date; every
  further semantic date of the record lives in the date-component
  detail table below, never in a delimiter list or aligned prose.
...
- **Per-basis value grammars.** Exact dates are `YYYY-MM-DD` (real
  ...
  source states a window. **Exact precision may never be degraded**: a
...
- **Date-component detail table (fixed; joinable; stable-identity;
  required for every record with more than one supported semantic
  date — cardinality corrected R2.8):** one row per **distinct
  supported semantic date**, in the shared registries, carrying a stable
  component identity so that multiple dates of the **same basis** with
  different roles/scopes are separately representable (the pre-R2.8
...
- `official-mutable` detail:
  ...
  base row's Source date field, where it is valid only as
  `edition:YYYY-YY` — must use the pinned `YYYY-YY` season grammar
  below, never any other season syntax)
...
and date-component rows, the `official-mutable` detail row's
Publication identity/date or season field, and any other structured
season value), the season is exactly `YYYY-YY`:
...
`Search record ID | Subject class | Subject historical LEAF or — | Subject historical fragment ID or — | Subject scenario or — | Subject scenario fragment ID or — | Authority/provenance class searched | Source identity | Source record ID or — | Canonical URL or authenticated provenance identifier or — | Binary/version identity or — | Binary size bytes or — | Binary SHA-256 or — | Binary pagination or — | Binary signature/as-of or — | Exact locator/query/provision | Search method | Search-set ID or — | Search cutoff timestamp | Result | Result linkage or — | Result details | Limitations or — | Verifier identity | Verification session ID | Verification date | Search status | Search version`

Field rules (binding; the composite `Size/hash/pagination/signature`
and `Current status/version` fields split into separate parseable
columns — R2.8):

- `Search record ID` — `SM2-<NNNN>`, unique, monotonically allocated.
- `Subject class` — closed vocabulary `XW2-DISP` | `SXW2-DISP`
  (§15.9.4), selecting which subject fields are required and which are
  `—`, exactly as the polymorphic `DISP` schema does.
- `Subject historical LEAF` / `Subject historical fragment ID` —
  required for `XW2-DISP` (the §15.9.1 published LEAF ID and the
  §15.9.3 fragment ID the search concerns; both must resolve); each
  exactly `—` for `SXW2-DISP`.
- `Subject scenario` / `Subject scenario fragment ID` — required for
  `SXW2-DISP` (the §15.9.8 `scenario-<n>` and `scenario-<n>:F<m>`; both
  must resolve); each exactly `—` for `XW2-DISP`. (SXW2 has no
  `unsupported-residual`, so an `SXW2-DISP`-subject search never
  supports a disposition or clears a finding.)
- `Authority/provenance class searched` — one §15.9.5 class or one
  §15.9.6 provenance type (e.g., `CBA`, `BYL`, `NBA`,
  `ops-provenance`).
- `Source identity` — the exact named source searched (never a vague
  class like "official web surfaces").
- `Source record ID` — the current `SRC2-…` record where one exists;
  `—` only where no `SRC2-…` record exists for the searched source.
- `Canonical URL or authenticated provenance identifier` — `—` only
  where neither exists (e.g., an attestation-availability check).
- `Binary/version identity` — the artifact edition/version searched
  (e.g., a PDF edition); `—` only for non-artifact sources.
- `Binary size bytes` — the searched artifact's size in bytes
  (integer); `—` only for non-artifact sources.
- `Binary SHA-256` — the searched artifact's 64-lowercase-hex SHA-256;
  `—` only for non-artifact sources.
- `Binary pagination` — the searched artifact's pagination (e.g., page
  count and printed-to-PDF offset); `—` only for non-artifact sources.
- `Binary signature/as-of` — the artifact's signature/as-of
  information; `—` where none applies.
- `Exact locator/query/provision` — the exact provision(s) inspected
  or the exact query run; never `—`.
- `Search method` — how the search was performed (full-text sweep,
  provision read, query, etc.); never `—`.
...
Rules (all binding): `Search set ID` is `SS2-<NNNN>`, unique, monotonic.
`Subject class` is `XW2-DISP` | `SXW2-DISP`. `Subject LEAF or scenario`
and `Subject fragment ID` name the exact fragment (LEAF + `:F` for
`XW2-DISP`; `scenario-<n>` + `:F` for `SXW2-DISP`), both resolving.
`Required source classes` is the closed, `", "`-separated set that must
be covered before an `unsupported-residual` disposition — at least the
signed CBA (`CBA`), the controlling By-Laws (`BYL`) where the mechanic
is plausibly operational/governance material, the official NBA
explanatory/annual surfaces (`NBA`), and the first-party
operational-provenance availability check (`ops-provenance`) — plus any
further class the unit's reasoning identifies as plausibly controlling.
...
this set (zero orphan/stale members). `Coverage assessment` is the
deterministic finding: every required source class has ≥1 current member
that is adequate (below) **and** whose `Result` is not `inconclusive`.
`Adequacy result` is a closed vocabulary `adequate-coverage` |
`inadequate-coverage` — `adequate-coverage` **only** when every required
class is so covered; any required class uncovered, or covered only by an
`inconclusive` or otherwise inadequate member, is `inadequate-coverage`.
`Set status`/`Set version` are `current` | `superseded` / integer. An
`unsupported-residual` `DISP` detail row's `Search-set ID` must name a
current `SS2-…` record with `Adequacy result = adequate-coverage`; an
`inadequate-coverage` set — or an `inconclusive` member for a required
class — can **never** support an `unsupported-residual` disposition or
clear a `blocked-unsupported-obligation` finding.
...
1. **Required before `unsupported-residual`:** a disposition may rely
   ...
   — its members covering, at least: the signed CBA, the controlling
   By-Laws where the mechanic is plausibly operational/governance
   material, the official NBA explanatory and annual-value surfaces,
   and the first-party operational-provenance availability check —
   plus any further class the performing unit's own reasoning
   identifies as plausibly controlling. The `DISP` detail row lists
   every relied-on `SM2-…` ID and names the current `SS2-…` set.
...
- **Scenario-fragment ID:** `scenario-<n>:F<m>` — the scenario
  ...
  a LEAF fragment.
- **Scenario-fragment inventory:** the §15.9.3 historical-fragment
  inventory, bundle, normalized-scope, and reconciliation contracts
  apply **analogously** to scenario fragments (the historical scenario
  replacing the historical LEAF, `scenario-<n>:F<m>` replacing
  `<LEAF ID>:F<n>`, `SXW2-…` edges replacing `XW2-…` edges), except that
  the SXW2 terminal vocabulary is only `invalid`/`no-successor` and no
  `unsupported-residual`/`SM2`/search-set machinery applies. R7 declares
  the scenario-fragment inventories when it builds the `SXW2-…`
  crosswalk; R2.8 mints no concrete scenario-fragment record. The crosswalk rules of §15.9.3 apply analogously: bipartite
...
7. No broken forward reference (every referenced ID exists and
   parses).
8. Every reference update landed in the **same commit** as the
   `AMEND` that made it necessary.
...
11. Every current `BLK-…` blocked finding that is `resolved` names a
    current `accepted` `RES-…` resolution whose `Independent checker
    identity` differs from its `Maker/proposer identity` (no maker
    self-acceptance), with a pinned acceptance commit/receipt; no `open`
...
disposition via one terminal edge or one governed `BND-…` bundle,
bidirectional resolution, kind ⇔ edge-type compatibility, and zero
`open` `BLK-…` `blocked-unsupported-obligation` findings — every one
carrying a current independently `accepted` `RES-…` resolution with a
checker distinct from the maker, and no unstructured waiver), and verify
...
| CBA2-A03 | Trade-salary value adjustments — Are outgoing and incoming trade-salary values adjusted exactly as the signed text requires (non-guarantees, base-year, poison pill, bonuses, minimum contracts)? | `CBA2-A03.1`–`CBA2-A03.8` (8) | GROUP anchor; no obligation, verdict, method, locator, or evidence |
| CBA2-A04 | Trade bonus — Are trade bonuses limited, triggered, allocated, amended, and charged exactly as the signed text requires? | `CBA2-A04.1`–`CBA2-A04.8` (8) | GROUP anchor; no obligation, verdict, method, locator, or evidence |
...
| CBA2-A07 | Sign-and-trade conditions — Is every VII §8(e)(1) sign-and-trade condition independently enforced? | `CBA2-A07.1`–`CBA2-A07.9` (9) | GROUP anchor; no obligation, verdict, method, locator, or evidence |
| CBA2-A08 | Cash in trades — Are the separate annual cash limits, the signing-bonus-as-cash rule, the Salary-Cap-Year charging rule, and the no-Team-Salary-effect rule enforced? | `CBA2-A08.1`–`CBA2-A08.5` (5) | GROUP anchor; no obligation, verdict, method, locator, or evidence |
...
| CBA2-A10 | Extension-and-trade — Are the VII §8(e)(2) mechanism, window, term, salary, raise, and six-month restrictions enforced? | `CBA2-A10.1`–`CBA2-A10.8` (8) | GROUP anchor; no obligation, verdict, method, locator, or evidence |
...
| CBA2-A02.8 | Expanded TPE limit: the Replacement Players' aggregate post-trade Salaries for the then-current Salary Cap Year may not exceed max(min(200% × O + K, 100% × O + A), 125% × O + K), where O is the aggregated pre-trade Salaries of the Traded Players, K is the $250,000 allowance as governed by CBA2-A02.12, and A = $7,500,000 × (current Salary Cap ÷ 2023-24 Salary Cap); the formula is canonical and derived season boundaries are never hard-coded | CBA, NBA, DERIVED | SCEN | STATIC | EV2-0009, EV2-0010, EV2-0011, EV2-0012 | XW2-0004, XW2-0008, XW2-0011 | 2026-27 derived values: A = $9.095710M (unrounded 9.0957095...); crossovers A−K = $8.845710M and 4(A−K) = $35.382838M; rounding only at the rule-defined final step. |
...
| CBA2-A03.4 | Sign-and-trade base-year adjustment: when a Qualifying or Early Qualifying Veteran Free Agent signs with his prior Team in connection with a VII §8(e) trade agreement, the Team's post-signing Team Salary is above the Salary Cap, and the new first-season Salary plus Unlikely Bonuses exceeds what a Non-Qualifying Veteran Free Agent signing under VII §6(b)(2) could have received, then for the assignor's TPE calculation the player's Salary is deemed the greater of (i) the Salary for the last Season of his preceding Contract or (ii) 50% of the first-season Salary of the new Contract; a prior one-year minimum Contract's Salary includes any League-reimbursed portion | CBA | SCEN | — | EV2-0022 | XW2-0020, XW2-0059 | Single conjunctive trigger producing one deemed-salary result (ATOM keep); affects the assignor side only. |
...
| CBA2-A03.6 | Poison-pill input assumptions: for the CBA2-A03.5 average, extended-term Salary expressed as a percentage of the Salary Cap is determined assuming the first extended-term Salary Cap equals 104.5% of the Salary Cap at the proposed trade and that the player meets no Higher Max Criteria in his fourth Season; extended-term Salary plus Unlikely Bonuses exceeding the applicable Maximum Annual Salary under that assumption is determined per the Article II §7(c) deemed amendments | CBA | SCEN | — | EV2-0024 | new (DR2-0044) | — |
...
| CBA2-A03.8 | Minimum Exception acquisition: a Contract meeting the VII §6(i) Minimum Player Salary Exception shape (≤2 Seasons at the applicable Minimum Player Salary, no bonuses of any kind) may be acquired by assignment through that Exception; the acquisition consumes no Traded Player Exception or room capacity (effective $0 incoming trade value), while the assignor's TPE is still calculated under VII §6(j)(6), whose unearned-Base-Compensation reduction excludes any League-reimbursed portion of a one-year minimum Contract | CBA, INFERRED | SCEN | — | EV2-0027, EV2-0028, EV2-0029 | XW2-0039, XW2-0040 | Minimum-contract shape, proration, and subsidy rules remain C-family (R4/R5). |
| CBA2-A04.1 | Trade-bonus amount cap and form: an Exhibit 4 trade bonus may be expressed only as a specified percentage of, or a specified dollar amount capped by a specified percentage of, the Base Compensation remaining to be earned under the Contract at the time of the trade (excluding an unexercised Option Year), and may never exceed 15% of that remaining Base Compensation | CBA | SCEN | — | EV2-0030 | XW2-0026, XW2-0031, XW2-0035 | The basis is Base Compensation remaining to be earned — not 'guaranteed' Base Compensation (source-law correction recorded by R3; see the amendment log). |
| CBA2-A04.2 | Single payability: a trade bonus is payable only the first time the Contract is traded and never more than once; for a Contract signed in connection with a VII §8(e) sign-and-trade agreement, the bonus does not apply to the initial trade and is payable only on the second trade | CBA | LIFECYCLE | SCEN | EV2-0031 | XW2-0027, XW2-0032 | Extension-added bonuses follow XXIV §2(a)(iv)'s first-trade-after-Extension variants. |
| CBA2-A04.3 | Trade-bonus allocation: an amount earned as a bonus upon trade is a VII §3(b)(1)(ii) signing bonus allocated over the then-current and any remaining Salary Cap Years of the Contract in proportion to each year's lack-of-skill-protected percentage of Base Compensation at allocation (only over years preceding an ETO's Effective Season); with zero qualifying protection, the entire bonus is allocated to the Salary Cap Year of the trade | CBA | SCEN | — | EV2-0032 | XW2-0028, XW2-0036 | The general signing-bonus allocation regime is C-family (historical C18; R5 records the crosswalk); this LEAF owns the trade-earned application. |
| CBA2-A04.4 | Rookie Scale 120% deemed amendment: if a trade of a Rookie Scale Contract would, because of a trade bonus, cause Salary plus Unlikely Bonuses for the trade year to exceed 120% of the applicable Rookie Scale Amount, the trade bonus is deemed amended to the extent necessary to reduce Salary plus Unlikely Bonuses to that 120% limit | CBA | SCEN | — | EV2-0033 | XW2-0025, XW2-0037 | No general veteran maximum-salary reduction of a trade bonus is expressed in the signed text; Article II §7 caps amounts at signing/renegotiation/extension only (source-law correction recorded by R3). |
| CBA2-A04.5 | Reduction amendment: in connection with a trade, a Contract may be amended to reduce (including to zero) a trade bonus that has not previously been earned, subject to the XXIV §2(a)(ii) cap and §2(a)(iii)(A) forms; a Contract with no trade bonus may not be amended to add one except simultaneously with an Extension under XXIV §2(a)(iv) | CBA | SCEN | — | EV2-0034 | XW2-0033 | — |
...
| CBA2-A04.7 | Assignor payment: the player is entitled to receive an earned trade bonus from the assignor Team within 30 days of the trade (UPC Exhibit 4) | CBA | SCEN | — | EV2-0036 | XW2-0029, XW2-0038 | BYL 4.04(c) permits the parties to reallocate contract obligations by agreement; the Exhibit 4 default is assignor payment. |
| CBA2-A04.8 | Receiving-side salary effect: the allocated trade-bonus amounts form part of the player's Salary, so the then-current year's allocation increases the player's post-assignment Salary for the assignee's VII §6(j) calculations and the assignee's Team Salary | CBA | SCEN | — | EV2-0037 | XW2-0030 | — |
| CBA2-A05.1 | Apron restriction test: a Team may not engage in a Transaction Restrictions Table transaction if, immediately following the transaction, its Apron Team Salary for the Salary Cap Year would exceed the row's Applicable Apron Level | CBA | SCEN | — | EV2-0038 | XW2-0041, XW2-0062, XW2-0064, XW2-0066, XW2-0068, XW2-0070, XW2-0072, XW2-0074, XW2-0076, XW2-0078, XW2-0104 | Uses post-transaction Apron Team Salary (VII §2(e)(1)); the Apron Team Salary computation is C-family (R4). Threshold status alone is never the test. |
| CBA2-A05.2 | Apron hard cap: a Team that engages in a Transaction Restrictions Table transaction may not, for the remainder of that Salary Cap Year, have an Apron Team Salary exceeding the row's Applicable Apron Level | CBA | LIFECYCLE | SCEN | EV2-0039 | XW2-0010, XW2-0042, XW2-0080, XW2-0081 | The hard cap persists as team state and constrains later transactions. |
...
| CBA2-A05.13 | Row K: signing a player using the Taxpayer Mid-Level Salary Exception carries an Applicable Apron Level of the Second Apron Level | CBA | SCEN | — | EV2-0050 | XW2-0077 | — |
| CBA2-A05.14 | TMLE-usage bar: for each Salary Cap Year beginning with 2024-25, a Team that has signed a Player Contract using the Taxpayer Mid-Level Salary Exception during that Salary Cap Year may not engage in any transaction in rows A through F of the Transaction Restrictions Table (rows A through E for the 2023-24 Salary Cap Year) | CBA | SCEN | LIFECYCLE | EV2-0051 | new (DR2-0045) | — |
...
| CBA2-A07.5 | Sign-and-trade exception bar: the Contract may not be signed pursuant to the Non-Taxpayer Mid-Level Salary Exception or the Mid-Level Salary Exception for Room Teams (VII §8(e)(1)(iii)) | CBA | SCEN | — | EV2-0063 | XW2-0118 | — |
...
| CBA2-A07.9 | Exhibit 6 bar: a Contract or Extension entered into under VII §8(e)(1) or (2) may not contain an Exhibit 6 (physical-examination validity condition); the Teams may instead condition the trade itself on a physical performed per NBA procedures (VII §8(e)(3)) | CBA | SCEN | — | EV2-0067 | new (DR2-0046) | The physical-examination outcome itself is an external determination consumed by the L-family (R6). |
| CBA2-A08.1 | Cash-paid annual limit: a Team may pay, in connection with all trades occurring during a Salary Cap Year, no more than an aggregate 5.15% of that year's Salary Cap in cash; amounts received are never netted against amounts paid | CBA | SCEN | LIFECYCLE | EV2-0068 | XW2-0101, XW2-0106, XW2-0108 | The derived dollar limit is a season parameter recomputed by the S-family (R6). |
| CBA2-A08.2 | Cash-received annual limit: a Team may receive, in connection with all trades occurring during a Salary Cap Year, no more than an aggregate 5.15% of that year's Salary Cap in cash (including reimbursement of Compensation obligations for players being acquired); amounts paid are never netted against amounts received | CBA | SCEN | LIFECYCLE | EV2-0069 | XW2-0102, XW2-0107, XW2-0109 | — |
...
| CBA2-A08.4 | Salary-Cap-Year charging: cash paid or received directly or indirectly in connection with one or more trades occurring during a Salary Cap Year counts against that Salary Cap Year's limits — conditional cash tied to a traded asset is charged to the Salary Cap Year of the trade, not the later payment date | CBA | SCEN | LIFECYCLE | EV2-0071 | XW2-0111 | The re-trade attribution/accounting mechanics for later re-trades of a conditional asset are an unsupported operational candidate (§15.9.6): no qualifying first-party provenance was located, no honest INFERRED chain completes from VII §8(a), and no active LEAF is minted for them. |
...
| CBA2-A10.4 | Extension-and-trade salary limit: for a VII §8(e)(2) Extension signed on or after the first day of the 2024-25 Salary Cap Year, first-year extended-term Salary (excluding Incentive Compensation) is capped at the greater of 120% of the last original-term year's Regular Salary or 120% of the Estimated Average Player Salary for the signing year (less any first-year extended-term Incentive Compensation); Likely and Unlikely Bonuses are capped at 120% of the last original-term year's respective amounts | CBA | SCEN | — | EV2-0077 | XW2-0120, XW2-0125 | Displaces the ordinary 140% rule of VII §7(a)(3)(i); the pre-2024-25 limit was 105% (VII §7(a)(3)(iii)(A)). |
| CBA2-A10.5 | Extension-and-trade raise limit: annual increases and decreases in a VII §8(e)(2) Extension are limited to 5% of the first extended-term year's Salary (excluding Incentive Compensation), Regular Salary, and per-bonus amounts respectively (VII §5(a)(4)) | CBA | SCEN | — | EV2-0078 | XW2-0122, XW2-0127 | — |
| CBA2-A10.6 | Extension-and-trade eligibility: a VII §8(e)(2) Extension must independently satisfy the VII §7(a) extension rules, including the §7(a)(1) anniversary/term timing, the §7(a)(2) renegotiation- and ETO-related bars, and the §7(a)(3)(v) requirement that the player will be a Qualifying Veteran Free Agent at the conclusion of his Contract | CBA | SCEN | — | EV2-0079 | XW2-0123 | — |
...
| CBA2-A12.1 | Pick-conveyance validity: a trade may convey only draft selections the conveying Team then owns (including conditionally owned selections), with the conveyed selections, their draft years, and all conditions stated as terms on the Trade Call and in the Trade Memorandum — a Team cannot convey a selection it merely expects to acquire | INFERRED | SCEN | LIFECYCLE | EV2-0084 | XW2-0094 | Inference from BYL 4.01(a), 4.02(a)-(b), and 4.05(a); undisclosed terms are unenforceable (BYL 4.02(a)). |
...
| CBA2-A12.5 | Unfreeze re-permission: if the Team is a Second Apron Team in fewer than two of the four following Salary Cap Years, the frozen first-round pick becomes tradable (conditionally or unconditionally) as of the day after the last day of the Regular Season of the third of those years in which the Team was not a Second Apron Team, and is not subject to a Draft Pick Penalty | CBA | LIFECYCLE | SCEN | EV2-0089 | new (DR2-0047) | The two-of-four Draft Pick Penalty branch and its draft-order consequence are L-family (R6). |
...
|---|---|---|---|---|
| CBA2-A01.1 | pending R7 | — | asOfDate; Salary Cap Year; team context | DR2-0001, DR2-0036 |
...
| CBA2-A02.8 | pending R7 | CBA2-A02.12, CBA2-A03.1 | Salary Cap Year; current and 2023-24 Salary Cap values | DR2-0002, DR2-0014 |
...
| CBA2-A03.1 | pending R7 | — | Trade date; elapsed Regular Season days; protection state | DR2-0003, DR2-0016 |
| CBA2-A03.2 | pending R7 | CBA2-A03.1 | Trade date; Regular Season calendar | DR2-0003 |
| CBA2-A03.3 | pending R7 | CBA2-A03.1 | Trade date; Regular Season calendar; next-year protection state | DR2-0003 |
| CBA2-A03.4 | pending R7 | — | Signing/trade date; Salary Cap Year; prior-contract salary | DR2-0003, DR2-0033 |
| CBA2-A03.5 | pending R7 | CBA2-A03.6 | Trade date vs extension date; Salary Cap Year | DR2-0003, DR2-0017 |
| CBA2-A03.6 | pending R7 | — | Salary Cap at proposed trade date | DR2-0003, DR2-0044 |
| CBA2-A03.7 | pending R7 | — | Preceding-season team/player performance data | DR2-0003, DR2-0018 |
| CBA2-A03.8 | pending R7 | CBA2-A03.1 | Transaction date; Salary Cap Year; player YOS | DR2-0003, DR2-0023 |
| CBA2-A04.1 | pending R7 | — | Trade date; earned-to-date Base Compensation | DR2-0004, DR2-0019 |
| CBA2-A04.2 | pending R7 | — | Trade history of the Contract | DR2-0004, DR2-0020 |
| CBA2-A04.3 | pending R7 | — | Trade date; protection percentages; ETO Effective Season | DR2-0004, DR2-0021 |
| CBA2-A04.4 | pending R7 | CBA2-A04.3 | Salary Cap Year; Rookie Scale Amount | DR2-0004 |
| CBA2-A04.5 | pending R7 | CBA2-A04.1 | Trade date; extension state | DR2-0004 |
| CBA2-A04.6 | pending R7 | CBA2-A04.5 | Trade date; renegotiation eligibility dates | DR2-0004 |
| CBA2-A04.7 | pending R7 | — | Trade date | DR2-0004, DR2-0022 |
| CBA2-A04.8 | pending R7 | CBA2-A04.3 | Trade date; Salary Cap Year | DR2-0004 |
...
| CBA2-A05.14 | pending R7 | CBA2-A05.13 | Salary Cap Year; TMLE-use history | DR2-0005, DR2-0045 |
...
| CBA2-A05.17 | pending R7 | — | Option/ETO state; pending league honors | DR2-0005 |
...
| CBA2-A07.1 | pending R7 | — | Prior-season roster history | DR2-0007 |
| CBA2-A07.2 | pending R7 | — | Contract term | DR2-0007 |
| CBA2-A07.3 | pending R7 | — | Contract term | DR2-0007 |
| CBA2-A07.4 | pending R7 | — | Protection state | DR2-0007 |
| CBA2-A07.5 | pending R7 | — | Signing mechanism | DR2-0007 |
| CBA2-A07.6 | pending R7 | — | Signing date; Regular Season start | DR2-0007 |
| CBA2-A07.7 | pending R7 | — | Salary Cap at signing; Higher Max status | DR2-0007, DR2-0046 |
| CBA2-A07.8 | pending R7 | — | Acquiring team's room/exception state | DR2-0007 |
| CBA2-A07.9 | pending R7 | — | Contract exhibits; trade conditions | DR2-0007, DR2-0046 |
| CBA2-A08.1 | pending R7 | CBA2-A08.4 | Salary Cap Year; cumulative cash-paid balance | DR2-0008, DR2-0035 |
| CBA2-A08.2 | pending R7 | CBA2-A08.4 | Salary Cap Year; cumulative cash-received balance | DR2-0008, DR2-0035 |
| CBA2-A08.3 | pending R7 | CBA2-A08.1, CBA2-A08.2 | Salary Cap Year of the trade | DR2-0008 |
| CBA2-A08.4 | pending R7 | — | Trade date; Salary Cap Year | DR2-0008 |
| CBA2-A08.5 | pending R7 | — | — | DR2-0008 |
| CBA2-A09.1 | pending R7 | — | Roster/list state at the Trade Call | DR2-0009 |
| CBA2-A10.1 | pending R7 | — | Extension/trade linkage | DR2-0010 |
| CBA2-A10.2 | pending R7 | CBA2-A10.1 | Contract term; Option/ETO state; Regular Season calendar | DR2-0010 |
| CBA2-A10.3 | pending R7 | CBA2-A10.1 | Extension signing date | DR2-0010 |
| CBA2-A10.4 | pending R7 | CBA2-A10.1 | Signing date; EAPS; prior-year salary | DR2-0010 |
| CBA2-A10.5 | pending R7 | CBA2-A10.4 | Extended-term salary schedule | DR2-0010 |
| CBA2-A10.6 | pending R7 | CBA2-A10.1 | Contract anniversaries; renegotiation/ETO history | DR2-0010 |
| CBA2-A10.7 | pending R7 | — | Extension/renegotiation signing date | DR2-0010 |
| CBA2-A10.8 | pending R7 | — | Trade date | DR2-0010 |
| CBA2-A11.1 | pending R7 | CBA2-A02.1, CBA2-A02.4, CBA2-A02.7, CBA2-A02.9 | Transaction date; per-team exception inventory | DR2-0011 |
| CBA2-A12.1 | pending R7 | — | Pick-ownership state at the Trade Call | DR2-0012 |
| CBA2-A12.2 | pending R7 | — | — | DR2-0012, DR2-0034 |
| CBA2-A12.3 | pending R7 | CBA2-A12.1, CBA2-A12.4 | Future-draft pick ownership and protection branches | DR2-0012, DR2-0034 |
| CBA2-A12.4 | pending R7 | — | Season-end Apron Team Salary history; seven-Season projection | DR2-0012 |
| CBA2-A12.5 | pending R7 | CBA2-A12.4 | Four-year Second Apron history | DR2-0012, DR2-0047 |

### 15.11 Historical crosswalk (created by R3; A family)

Typed edges from published v1.1 LEAFs (meanings fixed at commit `9814939c`, file SHA-256 `4a0760c8…`) to active v2 LEAFs, per §15.9.3. R3 recorded the A-family edges; R4–R6 extend this section in place. The published historical A-series population is exactly 89 LEAFs (recomputed mechanically in the R3 receipt); 88 receive R3 edges below and one (`CBA-A01.4`) carries a named later-family deferral. Historical verdicts never transfer.

| Edge ID | Historical v1.1 LEAF | Active v2 LEAF or — | Edge type | Scope/relationship | Decision record |
|---|---|---|---|---|---|
| XW2-0001 | CBA-A01.1 | CBA2-A01.1 | `merge` | Whole obligation (independent derivation of the defined salary quantities); absorbed with CBA-A01.2 and the CBA-A01.3 fragment | DR2-0036 |
| XW2-0002 | CBA-A01.2 | CBA2-A01.1 | `merge` | Whole obligation (player-compensation ledger separate from Team Salary) — the same non-substitution rule stated for the Compensation/Salary pair | DR2-0036 |
| XW2-0003 | CBA-A01.3 | CBA2-A01.1 | `partial-overlap` | Context-dependent-derivation fragment only; the 'no shared mutable salary field' clause is implementation instruction (process material, not carried); the explicit-date fragment is deferred to R6 (see deferrals) | DR2-0036 |
| XW2-0004 | CBA-A02.1 | CBA2-A02.8 | `merge` | Whole obligation (implement the official Expanded TPE formula; never hard-code displayed boundaries); absorbed with CBA-A02.5 and the CBA-A02.4 formula fragment | DR2-0014 |
| XW2-0005 | CBA-A02.2 | CBA2-A02.12 | `merge` | Whole obligation (allowance test uses post-assignment Apron Team Salary); absorbed with CBA-A02.8 | DR2-0015 |
| XW2-0006 | CBA-A02.3 | — | `process-only` | Correction/process note about remembered five-tier structure; the operative no-hard-coding contract lives in CBA2-A02.8's requirement | DR2-0039 |
| XW2-0007 | CBA-A02.4 | CBA2-A02.7 | `split` | Expanded-path structure fragment (one or more outgoing/incoming; simultaneous), wholly owned by the target | DR2-0002 |
| XW2-0008 | CBA-A02.4 | CBA2-A02.8 | `partial-overlap` | Formula fragment, folded with owners CBA-A02.1/CBA-A02.5 | DR2-0014 |
| XW2-0009 | CBA-A02.4 | CBA2-A05.7 | `partial-overlap` | Row-E apron fragment (must land at/below First Apron), folded with CBA-A12.5's row fragment | DR2-0027 |
| XW2-0010 | CBA-A02.4 | CBA2-A05.2 | `partial-overlap` | Hard-cap fragment (creates First Apron hard cap), folded with CBA-A13 | DR2-0025 |
| XW2-0011 | CBA-A02.5 | CBA2-A02.8 | `merge` | Whole obligation (max ITS formula max(min(2O+K, O+A), 1.25O+K)) | DR2-0014 |
| XW2-0012 | CBA-A02.6 | — | `process-only` | UI tier-derivation instruction; the no-fixed-boundaries contract lives in CBA2-A02.8's requirement | DR2-0039 |
| XW2-0013 | CBA-A02.7 | CBA2-A02.6 | `equivalent` | Whole obligation (110% Transition TPE existed only for 2023-24; historical simulations only) | DR2-0002 |
| XW2-0014 | CBA-A02.8 | CBA2-A02.12 | `merge` | Whole obligation ($250K allowance reduced to zero above the First Apron on post-assignment Apron Team Salary) | DR2-0015 |
| XW2-0015 | CBA-A03.1 | CBA2-A03.1 | `merge` | Whole obligation (in-season non-guaranteed OTS = salary less unearned/unprotected); the 'test at 0%, 25%, 100% elapsed' clause is testing instruction (process material, not carried) | DR2-0016 |
| XW2-0016 | CBA-A03.2 | CBA2-A03.1 | `merge` | Whole obligation (window 1: count the protected amount) — the §6(j)(6) general reduction applied pre-season (example case (W)) | DR2-0016 |
| XW2-0017 | CBA-A03.3 | CBA2-A03.1 | `merge` | Whole obligation (window 2: salary less unearned/unprotected base compensation) — the general reduction applied in-season (example case (X)) | DR2-0016 |
| XW2-0018 | CBA-A03.4 | CBA2-A03.2 | `equivalent` | Whole obligation (window 3: January 8 through Regular Season end deemed fully protected) | DR2-0003 |
| XW2-0019 | CBA-A03.5 | CBA2-A03.3 | `equivalent` | Whole obligation (window 4: post-season lesser-of rule) | DR2-0003 |
| XW2-0020 | CBA-A04 | CBA2-A03.4 | `merge` | Whole obligation (sign-and-trade base-year OTS adjustment including reimbursed minimum); absorbed with CBA-A10.3's base-year fragment | DR2-0033 |
| XW2-0021 | CBA-A05.1 | CBA2-A03.5 | `merge` | Whole obligation (trade before extension begins triggers poison-pill treatment) — the trigger half of §8(g) | DR2-0017 |
| XW2-0022 | CBA-A05.2 | CBA2-A03.5 | `merge` | Whole obligation (deemed average annual salary for acquiring-side fit, including option treatment via VII §9(a)(2)) | DR2-0017 |
| XW2-0023 | CBA-A06.1 | CBA2-A03.7 | `merge` | Whole obligation (likelihood based on preceding season; team-related criteria re-test on team change) | DR2-0018 |
| XW2-0024 | CBA-A06.2 | CBA2-A03.7 | `merge` | Whole obligation (per-team re-test makes OTS and ITS diverge) | DR2-0018 |
| XW2-0025 | CBA-A07.1 | CBA2-A04.4 | `equivalent` | Whole obligation (trade bonus cannot push Salary plus Unlikely Bonuses above 120% of the Rookie Scale Amount) | DR2-0004 |
| XW2-0026 | CBA-A07.2 | CBA2-A04.1 | `partial-overlap` | Fixed/percentage form and 15% cap fragment, folded with owners CBA-A07.4/CBA-A07.7 | DR2-0019 |
| XW2-0027 | CBA-A07.2 | CBA2-A04.2 | `partial-overlap` | Paid-once fragment, folded with owner CBA-A07.5 | DR2-0020 |
| XW2-0028 | CBA-A07.2 | CBA2-A04.3 | `partial-overlap` | Then-current-and-remaining protected-percentage allocation fragment, folded with the CBA-A07.8 fragment | DR2-0021 |
| XW2-0029 | CBA-A07.2 | CBA2-A04.7 | `partial-overlap` | Generally-paid-by-sender fragment, folded with owner CBA-A07.9 | DR2-0022 |
| XW2-0030 | CBA-A07.3 | CBA2-A04.8 | `equivalent` | Whole obligation (current-year allocated portion increases ITS and receiving Team Salary) | DR2-0004 |
| XW2-0031 | CBA-A07.4 | CBA2-A04.1 | `merge` | Whole obligation (15% cap; fixed, percentage, or lesser-of forms) | DR2-0019 |
| XW2-0032 | CBA-A07.5 | CBA2-A04.2 | `merge` | Whole obligation (triggered once; initial sign-and-trade/extend-and-trade does not consume; later trade can) | DR2-0020 |
| XW2-0033 | CBA-A07.6 | CBA2-A04.5 | `split` | Reduction/waiver-permission fragment, wholly owned by the target | DR2-0004 |
| XW2-0034 | CBA-A07.6 | CBA2-A04.6 | `split` | Six-month renegotiation-restriction fragment, wholly owned by the target | DR2-0004 |
| XW2-0035 | CBA-A07.7 | CBA2-A04.1 | `merge` | Whole obligation (the percentage-calculation basis); the published 'guaranteed base compensation' qualifier is not carried — the signed basis is Base Compensation remaining to be earned (source-law correction recorded in the amendment log) | DR2-0019 |
| XW2-0036 | CBA-A07.8 | CBA2-A04.3 | `partial-overlap` | Allocation fragment (VII §3(b)(1)(ii) basis), folded with the CBA-A07.2 fragment | DR2-0021 |
| XW2-0037 | CBA-A07.8 | CBA2-A04.4 | `partial-overlap` | Maximum-reduction fragment; the supported content is the Rookie Scale VIII §1(d) deemed amendment — the published generic 'annual maximum salary' reduction is not expressed in the signed text (source-law correction recorded in the amendment log) | DR2-0004 |
| XW2-0038 | CBA-A07.9 | CBA2-A04.7 | `merge` | Whole obligation (sending team normally pays; receiving team carries the allocation) | DR2-0022 |
| XW2-0039 | CBA-A08.1 | CBA2-A03.8 | `partial-overlap` | Acquisition/$0-ITS fragment only; the contract-shape and proration fragments are deferred to R4 (see deferrals) | DR2-0023 |
| XW2-0040 | CBA-A08.2 | CBA2-A03.8 | `merge` | Whole obligation (qualifying minimum counts as $0 incoming while the sender retains outgoing value) | DR2-0023 |
| XW2-0041 | CBA-A09.1 | CBA2-A05.1 | `partial-overlap` | Type/apron-dependent-availability fragment, folded into the general post-transaction test | DR2-0027 |
| XW2-0042 | CBA-A09.1 | CBA2-A05.2 | `partial-overlap` | Hard-cap-effects fragment, folded with CBA-A13 | DR2-0025 |
| XW2-0043 | CBA-A09.2 | CBA2-A02.9 | `split` | Room-path limit fragment (room + $250K), wholly owned by the target | DR2-0002 |
| XW2-0044 | CBA-A09.2 | CBA2-A02.10 | `split` | Room-path exclusivity fragment (no simultaneous TPE combination), wholly owned by the target | DR2-0002 |
| XW2-0045 | CBA-A09.3 | CBA2-A02.1 | `partial-overlap` | Structure fragment (one outgoing, one or more incoming), folded with the CBA-A10.1 fragment | DR2-0028 |
| XW2-0046 | CBA-A09.3 | CBA2-A02.2 | `split` | Money-limit fragment (100% OTS + $250K), wholly owned by the target | DR2-0002 |
| XW2-0047 | CBA-A09.3 | CBA2-A02.3 | `partial-overlap` | Twelve-month-expiry fragment, folded with the CBA-A09.5 reminder | DR2-0029 |
| XW2-0048 | CBA-A09.3 | CBA2-A05.8 | `partial-overlap` | First-Apron-timing fragment, folded with CBA-A12.6/CBA-A09.5 | DR2-0030 |
| XW2-0049 | CBA-A09.4 | CBA2-A02.4 | `split` | Structure fragment (multiple outgoing aggregated; simultaneous), wholly owned by the target | DR2-0002 |
| XW2-0050 | CBA-A09.4 | CBA2-A02.5 | `split` | Money-limit fragment (100% aggregate OTS + $250K), wholly owned by the target | DR2-0002 |
| XW2-0051 | CBA-A09.4 | CBA2-A05.10 | `partial-overlap` | Row-H apron fragment (must land at/below Second Apron), folded with CBA-A12.7/CBA-A10.1 | DR2-0031 |
| XW2-0052 | CBA-A09.5 | CBA2-A05.8 | `partial-overlap` | Row-F aged-TPE timing fragment (whole point of the row), folded with CBA-A12.6 | DR2-0030 |
| XW2-0053 | CBA-A09.5 | CBA2-A02.3 | `partial-overlap` | One-year non-simultaneous-expiration reminder fragment, folded with the CBA-A09.3 fragment | DR2-0029 |
| XW2-0054 | CBA-A10.1 | CBA2-A02.1 | `partial-overlap` | Standard-path multi-incoming permission fragment, folded with the CBA-A09.3 fragment | DR2-0028 |
| XW2-0055 | CBA-A10.1 | CBA2-A05.10 | `partial-overlap` | Row-H scope fragment ('no aggregation' bars combining outgoing under the Aggregated path only), folded with CBA-A12.7/CBA-A09.4 | DR2-0031 |
| XW2-0056 | CBA-A10.2 | CBA2-A06.1 | `partial-overlap` | Two-month-bar fragment, folded with the CBA-A10.3 re-aggregation fragment | DR2-0032 |
| XW2-0057 | CBA-A10.2 | CBA2-A06.2 | `split` | December 16 carve-out fragment, wholly owned by the target | DR2-0032 |
| XW2-0058 | CBA-A10.3 | CBA2-A06.1 | `partial-overlap` | Re-aggregation-bar fragment (the same §6(j)(4)(i) rule), folded with CBA-A10.2 | DR2-0032 |
| XW2-0059 | CBA-A10.3 | CBA2-A03.4 | `partial-overlap` | Base-year-adjustment fragment, folded with owner CBA-A04 | DR2-0033 |
| XW2-0060 | CBA-A11 | CBA2-A11.1 | `equivalent` | Whole obligation (per-team evaluation and component-trade decomposition, express structure plus INFERRED procedure) | DR2-0011 |
| XW2-0061 | CBA-A12.1 | CBA2-A05.3 | `split` | Row-A assignment fragment (BAE → First Apron), wholly owned by the target | DR2-0005 |
| XW2-0062 | CBA-A12.1 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment, folded into the general test | DR2-0027 |
| XW2-0063 | CBA-A12.2 | CBA2-A05.4 | `split` | Row-B assignment fragment (NTMLE → First Apron), wholly owned by the target | DR2-0005 |
| XW2-0064 | CBA-A12.2 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0065 | CBA-A12.3 | CBA2-A05.5 | `partial-overlap` | Row-C assignment fragment, folded with CBA-A12.10 | DR2-0024 |
| XW2-0066 | CBA-A12.3 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0067 | CBA-A12.4 | CBA2-A05.6 | `split` | Row-D assignment fragment (qualifying waived-player signing → First Apron), wholly owned by the target | DR2-0005 |
| XW2-0068 | CBA-A12.4 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0069 | CBA-A12.5 | CBA2-A05.7 | `partial-overlap` | Row-E assignment fragment, folded with the CBA-A02.4 fragment | DR2-0027 |
| XW2-0070 | CBA-A12.5 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0071 | CBA-A12.6 | CBA2-A05.8 | `partial-overlap` | Row-F assignment fragment, folded with CBA-A09.5 | DR2-0030 |
| XW2-0072 | CBA-A12.6 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0073 | CBA-A12.7 | CBA2-A05.10 | `partial-overlap` | Row-H assignment fragment, folded with CBA-A09.4/CBA-A10.1 | DR2-0031 |
| XW2-0074 | CBA-A12.7 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0075 | CBA-A12.8 | CBA2-A05.12 | `split` | Row-J assignment fragment (sign-and-traded-contract TPE → Second Apron), wholly owned by the target | DR2-0005 |
| XW2-0076 | CBA-A12.8 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0077 | CBA-A12.9 | CBA2-A05.13 | `split` | Row-K assignment fragment (TMLE → Second Apron), wholly owned by the target | DR2-0005 |
| XW2-0078 | CBA-A12.9 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0079 | CBA-A12.10 | CBA2-A05.5 | `partial-overlap` | Row-C assignment fragment (receiving by sign-and-trade), folded with CBA-A12.3 | DR2-0024 |
| XW2-0080 | CBA-A12.10 | CBA2-A05.2 | `partial-overlap` | Hard-cap fragment (creates a First Apron hard cap), folded with CBA-A13 | DR2-0025 |
| XW2-0081 | CBA-A13 | CBA2-A05.2 | `merge` | Whole obligation (executing a restricted transaction creates the row-level hard cap; the published First-Apron-only wording under-stated the general rule); absorbed with the CBA-A12.10/CBA-A02.4/CBA-A09.1 fragments | DR2-0025 |
| XW2-0082 | CBA-A14.1 | CBA2-A05.15 | `merge` | Whole obligation (dual-year test gated by transaction type — rows E-J only) | DR2-0026 |
| XW2-0083 | CBA-A14.2 | CBA2-A05.15 | `merge` | Whole obligation (post-regular-season rows E-J transactions satisfy both years' applicable aprons) | DR2-0026 |
| XW2-0084 | CBA-A14.3 | CBA2-A05.17 | `equivalent` | Whole obligation (the next-year computation assumptions) | DR2-0005 |
| XW2-0085 | CBA-A14.4 | CBA2-A05.16 | `equivalent` | Whole obligation (such a transaction can hard-cap both years — the Subsequent-year hard cap) | DR2-0005 |
| XW2-0086 | CBA-A15.1 | — | `invalid` | The published row asserted an enforceable OPS-authority two-team asset-exchange requirement; secondary reporting was its only support, so the authority/enforceability claim is false under §15.9.5-§15.9.6. The reported mechanic is preserved as an unsupported operational candidate in §12.2 (never registrable or enforceable while unsupported) | DR2-0037 |
| XW2-0087 | CBA-A15.2 | — | `invalid` | False OPS-authority claim (two-other-team touch test); candidate preserved in §12.2 | DR2-0037 |
| XW2-0088 | CBA-A15.3 | — | `invalid` | False OPS-authority claim (stricter multi-team asset definitions); candidate preserved in §12.2 | DR2-0037 |
| XW2-0089 | CBA-A15.4 | — | `invalid` | False OPS-authority claim (deemed draft-rights status tests); candidate preserved in §12.2 | DR2-0037 |
| XW2-0090 | CBA-A15.5 | — | `invalid` | False OPS-authority claim (multi-team graph validation as an enforceable operational rule); candidate preserved in §12.2 | DR2-0037 |
| XW2-0091 | CBA-A16 | CBA2-A09.1 | `equivalent` | Whole obligation (open list room required at the trade, planned waiver notwithstanding); the active statement follows BYL 4.05(e)'s exact list scope (Active, Inactive, or Two-Way List) and same-transaction accounting | DR2-0009 |
| XW2-0092 | CBA-A17.1 | CBA2-A12.3 | `partial-overlap` | Stepien-availability fragment (pick state consumed by the Stepien test), folded with the CBA-A17.5/CBA-A17.6 owners; the general pick-ledger representation fragment is deferred to R6 (see deferrals) | DR2-0012 |
| XW2-0093 | CBA-A17.1 | CBA2-A12.4 | `partial-overlap` | Frozen/slid-status fragment (frozen-pick trading state); the slide/penalty representation is deferred to R6 (see deferrals) | DR2-0012 |
| XW2-0094 | CBA-A17.2 | CBA2-A12.1 | `equivalent` | Whole obligation (future picks must be identified and owned; no promising expected assets), re-grounded as an INFERRED chain from BYL 4.01(a)/4.02(a)-(b)/4.05(a) | DR2-0012 |
| XW2-0095 | CBA-A17.3 | — | `invalid` | False OPS-authority claim (protection/deferral combination bar and one-year deferral right as enforceable operational rules); the reported mechanics are preserved as unsupported operational candidates in §13.3; the real representation duty is deferred with the CBA-A17.1 fragment | DR2-0038 |
| XW2-0096 | CBA-A17.4 | — | `invalid` | False OPS-authority claim (seven-future-draft horizon); candidate preserved in §13.3 | DR2-0038 |
| XW2-0097 | CBA-A17.5 | CBA2-A12.2 | `split` | No-cash-sale fragment (BYL 7.03 first clause), wholly owned by the target | DR2-0034 |
| XW2-0098 | CBA-A17.5 | CBA2-A12.3 | `partial-overlap` | Stepien-test fragment (may-test, owned-first satisfaction, branch testing), folded with owner CBA-A17.6 | DR2-0034 |
| XW2-0099 | CBA-A17.6 | CBA2-A12.3 | `merge` | Whole obligation (protections evaluated across all possible conveyance branches — the 'may' semantics of the one BYL 7.03 test); absorbed with the CBA-A17.5 fragment | DR2-0034 |
| XW2-0100 | CBA-A17.7 | — | `invalid` | False OPS-authority claim (the 'two years after prior conveyance' limit presupposed the unsupported seven-draft horizon); candidate preserved in §13.3 | DR2-0038 |
| XW2-0101 | CBA-A18.1 | CBA2-A08.1 | `partial-overlap` | Sent-balance ledger fragment, folded with the CBA-A18.4/CBA-A18.5 fragments | DR2-0035 |
| XW2-0102 | CBA-A18.1 | CBA2-A08.2 | `partial-overlap` | Received-balance ledger fragment | DR2-0035 |
| XW2-0103 | CBA-A18.2 | CBA2-A05.11 | `partial-overlap` | Row-I assignment fragment (cash prohibited above the Second Apron), folded with owner CBA-A18.8 | DR2-0013 |
| XW2-0104 | CBA-A18.2 | CBA2-A05.1 | `partial-overlap` | Post-transaction-test fragment | DR2-0027 |
| XW2-0105 | CBA-A18.3 | CBA2-A08.3 | `equivalent` | Whole obligation (sending-team signing bonus treated as cash-in-trade) | DR2-0008 |
| XW2-0106 | CBA-A18.4 | CBA2-A08.1 | `partial-overlap` | Cap-indexed separate sent-limit fragment | DR2-0035 |
| XW2-0107 | CBA-A18.4 | CBA2-A08.2 | `partial-overlap` | Cap-indexed separate received-limit fragment | DR2-0035 |
| XW2-0108 | CBA-A18.5 | CBA2-A08.1 | `partial-overlap` | No-netting fragment applied to the paid direction | DR2-0035 |
| XW2-0109 | CBA-A18.5 | CBA2-A08.2 | `partial-overlap` | No-netting fragment applied to the received direction | DR2-0035 |
| XW2-0110 | CBA-A18.6 | CBA2-A08.5 | `equivalent` | Whole obligation (cash has no Team Salary effect), re-grounded as an INFERRED chain from VII §4(a)/§8(a) | DR2-0008 |
| XW2-0111 | CBA-A18.7 | CBA2-A08.4 | `partial-overlap` | Express Salary-Cap-Year charging fragment (VII §8(a)) only. The residual re-trade attribution/accounting fragment has no active owner: it is an unsupported operational candidate (§15.9.6) preserved in §12.12 — not registrable, not OPS, not INFERRED (no complete official chain exists), and not enforceable; disclosed in the R3 receipt and §19.3 | DR2-0008 |
| XW2-0112 | CBA-A18.8 | CBA2-A05.11 | `merge` | Whole obligation (paying cash is a Second Apron-limited transaction — row I); absorbed with the CBA-A18.2 fragment | DR2-0013 |
| XW2-0113 | CBA-A19.1 | CBA2-A07.1 | `equivalent` | Whole obligation (§8(e)(1)(i) roster condition) | DR2-0007 |
| XW2-0114 | CBA-A19.2 | CBA2-A07.6 | `equivalent` | Whole obligation (§8(e)(1)(v) pre-season completion) | DR2-0007 |
| XW2-0115 | CBA-A19.3 | CBA2-A07.2 | `split` | Minimum-term fragment (≥3 Seasons excluding options), wholly owned by the target | DR2-0007 |
| XW2-0116 | CBA-A19.3 | CBA2-A07.3 | `split` | Maximum-term fragment (≤4 Seasons), wholly owned by the target | DR2-0007 |
| XW2-0117 | CBA-A19.3 | CBA2-A07.4 | `split` | Year-1 full lack-of-skill protection fragment, wholly owned by the target | DR2-0007 |
| XW2-0118 | CBA-A19.4 | CBA2-A07.5 | `equivalent` | Whole obligation (§8(e)(1)(iii) exception bar — NTMLE and Room MLE) | DR2-0007 |
| XW2-0119 | CBA-A19.5 | CBA2-A07.8 | `equivalent` | Whole obligation (§8(e)(1)(vii) acquiring-team Room requirement) | DR2-0007 |
| XW2-0120 | CBA-A20.1 | CBA2-A10.4 | `partial-overlap` | 120%-measures fragment, folded with owner CBA-A20.3 | DR2-0010 |
| XW2-0121 | CBA-A20.1 | CBA2-A10.3 | `partial-overlap` | Length-limit fragment, folded with the CBA-A20.4 fragment | DR2-0010 |
| XW2-0122 | CBA-A20.1 | CBA2-A10.5 | `partial-overlap` | 5%-changes fragment, folded with the CBA-A20.4 fragment | DR2-0010 |
| XW2-0123 | CBA-A20.2 | CBA2-A10.6 | `split` | Ordinary-extension-eligibility fragment (§7(a) compliance), wholly owned by the target | DR2-0010 |
| XW2-0124 | CBA-A20.2 | CBA2-A10.2 | `split` | End-of-contract offseason-window fragment (§8(e)(2)(i)), wholly owned by the target | DR2-0010 |
| XW2-0125 | CBA-A20.3 | CBA2-A10.4 | `merge` | Whole obligation (starting salary capped at greater of 120% of prior regular salary or 120% of EAPS, adjusted for incentives); absorbed with the CBA-A20.1 fragment | DR2-0010 |
| XW2-0126 | CBA-A20.4 | CBA2-A10.3 | `partial-overlap` | Total-length fragment, folded with the CBA-A20.1 fragment | DR2-0010 |
| XW2-0127 | CBA-A20.4 | CBA2-A10.5 | `partial-overlap` | Annual-changes fragment, folded with the CBA-A20.1 fragment | DR2-0010 |
| XW2-0128 | CBA-A20.5 | CBA2-A10.1 | `split` | Linked-process fragment (extension and trade linked per §8(e)(2)), wholly owned by the target | DR2-0010 |
| XW2-0129 | CBA-A20.5 | CBA2-A10.7 | `split` | Six-month richer-pre-trade-extension fragment (outbound bar), wholly owned by the target | DR2-0010 |
| XW2-0130 | CBA-A20.5 | CBA2-A10.8 | `split` | Six-month richer-post-trade-extension fragment (inbound bar), wholly owned by the target | DR2-0010 |
| XW2-0131 | CBA-A21 | CBA2-A06.3 | `equivalent` | Whole obligation (minimum-stacking prohibition with its three conjunctive conditions and season-dependent classification) | DR2-0006 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SRC2-001 | `official-immutable` | 2023 NBA-NBPA Collective Bargaining Agreement (signed agreement, 2023 edition) | 2023-06-28 | <https://ak-static.cms.nba.com/wp-content/uploads/sites/4/2023/06/2023-NBA-Collective-Bargaining-Agreement.pdf> | `bf178ca0f2d64f9dfe6fde095d3ae43d576b12e19ce7a679618d632584f7ab32` | 2026-07-16T09:39:26Z | — | `agent:claude-code` | `session:r3-20260716-01` | 2026-07-16 | none | Signed 2023 edition; 2,850,534 bytes; 676 PDF pages |
| SRC2-002 | `official-immutable` | NBA Constitution and By-Laws, June 2024 edition | 2024-06-07 | <https://official.nba.com/wp-content/uploads/sites/4/2024/06/NBA-Consitution-By-Laws-June-2024.pdf> | `be4d2781fe8fddfc5bc9028214298f742789a949dade4ead26368a4336d32ccf` | 2026-07-16T09:40:09Z | — | `agent:claude-code` | `session:r3-20260716-01` | 2026-07-16 | Edition is dated to the month (JUNE 2024) on its cover; the day-precision publication date is taken from the artifact's embedded PDF creation/modification date (2024-06-07) | June 2024 edition; 422,247 bytes; 88 PDF pages |
| SRC2-003 | `official-mutable` | NBA Communications official release: NBA Salary Cap for 2023-24 season set at $136.021 million | 2023-06-30 | <https://pr.nba.com/nba-salary-cap-for-2023-24-season-set-at-136-021-million/> | `c162ae4a821c8ed38e1af37a75a5368d558ae941455210c0cd5b301d0e42329b` | 2026-07-16T09:59:50Z | — | `agent:claude-code` | `session:r3-20260716-01` | 2026-07-16 | Mutable webpage; hash is of the content retrieved at the recorded timestamp | Live pr.nba.com archive page as retrieved 2026-07-16 |
| SRC2-004 | `official-mutable` | NBA Communications official release: NBA sets Salary Cap for 2026-27 season at $164.961 million | 2026-06-30 | <https://pr.nba.com/2026-27-salary-cap/> | `cdc91324694aea16627b8e938d1c86c4865667e18e76d0beeb789d48628f4766` | 2026-07-16T09:59:51Z | — | `agent:claude-code` | `session:r3-20260716-01` | 2026-07-16 | Mutable webpage; hash is of the content retrieved at the recorded timestamp | Live pr.nba.com page as retrieved 2026-07-16 |
...
| Record ID | Publication identity/date or season | Exact values or text relied upon | Archive/snapshot reference or — |
|---|---|---|---|
| SRC2-003 | Official Release, June 30, 2023 (season 2023-24) | 2023-24 Salary Cap $136.021 million | — |
| SRC2-004 | Official Release, June 30, 2026 (season 2026-27) | 2026-27 Salary Cap $164.961 million; First Apron Level $209.015 million; Second Apron Level $221.686 million; Tax Level $200.428 million; Minimum Team Salary $148.465 million | — |

#### 15.12.4 EV2 authority-component evidence
...
| EV2-0012 | CBA2-A02.8 | DERIVED | — | EV2-0009, EV2-0010, EV2-0011 | Derivation from CBA VII §6(j)(1)(iv) with published Salary Cap inputs | A = $7,500,000 × (current Salary Cap ÷ 2023-24 Salary Cap); crossovers at O = A − K and O = 4(A − K) | Reproduces the season-scaled amount and the formula's regime boundaries from the signed formula and official inputs | 2026-27: A = 7.5 × 164.961/136.021 = $9.0957095M; A − K = $8.8457095M; 4(A − K) = $35.3828379M (values in millions; unrounded intermediates; dollar rounding only at the rule-defined final comparison) | Inputs are Salary Cap values only; every input resolves to EV2-0009/0010/0011 |
...
| EV2-0022 | CBA2-A03.4 | CBA | SRC2-001 | — | CBA VII §6(j)(5), p. 243 | If (x) a Qualifying Veteran Free Agent or Early Qualifying Veteran Free Agent and his prior Team enter into a Player Contract… in connection with an agreement to trade the Contract in accordance with Section 8(e) below, (y) the Team's Team Salary immediately following such Contract signing is above the Salary Cap, and (z) the new Contract… provides for Salary and Unlikely Bonuses… in excess of [Non-Bird capacity], then… the player's Salary shall be deemed equal to the greater of (i) the Salary for the last Season of his preceding Contract, or (ii) fifty percent (50%) of the Salary for the first Season of his new Contract… the player's prior Salary shall include the portion of the Minimum Player Salary, if any, that was reimbursed… | States the conjunctive base-year trigger, the greater-of deemed salary, and the reimbursed-minimum inclusion for the assignor's TPE calculation | — | — |
...
| EV2-0024 | CBA2-A03.6 | CBA | SRC2-001 | — | CBA VII §8(g)(i)-(ii), p. 265 | If the Contract provides for Base Compensation… expressed as a percentage of the Salary Cap…, the Base Compensation in the extended term shall be determined assuming that (a) the Salary Cap in the first Salary Cap Year covered by the extended term will equal one hundred four and one-half percent (104.5%) of the Salary Cap in effect at the time that the proposed trade would occur, and (b) the player does not meet any of the applicable Higher Max Criteria…; …[over-max amounts] shall be determined to be the Salary plus Unlikely Bonuses that would result from the deemed amendment(s) pursuant to Article II, Section 7(c)… | Fixes the extended-term salary inputs for the CBA2-A03.5 average under percentage-based and over-maximum extension shapes | — | — |
...
| EV2-0027 | CBA2-A03.8 | CBA | SRC2-001 | — | CBA VII §6(i), p. 240 | A Team may sign a player to, or acquire by assignment, a Player Contract, not to exceed two (2) Seasons in length, that provides for a Salary for the first Season equal to the Minimum Player Salary applicable to that player (with no bonuses of any kind)… | Grants the express acquisition-by-assignment authority for qualifying minimum Contracts | — | — |
| EV2-0028 | CBA2-A03.8 | CBA | SRC2-001 | — | CBA VII §6(j)(6)(ii), p. 244 | With respect to the assignment of a Player Contract that is a one-year Contract that provides for Salary equal to the Minimum Player Salary (with no bonuses of any kind), the player's unearned Base Compensation shall exclude the portion of the Minimum Player Salary, if any, that is reimbursed out of the League-wide benefits fund… | Keeps the assignor's outgoing value intact by excluding the reimbursed portion from the §6(j)(6) reduction | — | — |
| EV2-0029 | CBA2-A03.8 | INFERRED | SRC2-001 | EV2-0027 | CBA VII §6(m), p. 247; I §1(kkk), p. 9 | Other than in accordance with Section 6(j) above, a Team may not aggregate or combine any of the Exceptions… If a Team has more than one (1) Exception available at the same time, the Team shall have the right to choose which Exception it wishes to use to sign or acquire a player | Because §6(i) is its own acquisition Exception that a team may choose, an acquisition made through it is not made through §6(j) or cap room, so it consumes no TPE or room capacity — the effective incoming trade value is $0 | Inference chain: §6(i) express acquisition authority (EV2-0027) + §6(m) exception-selection right → no §6(j)/room capacity is consumed; the text does not use the phrase '$0 incoming salary' | — |
| EV2-0030 | CBA2-A04.1 | CBA | SRC2-001 | — | CBA XXIV §2(a)(ii) and §2(a)(iii)(A), pp. 414-15 | A trade bonus shall not exceed fifteen percent (15%) of the Base Compensation remaining to be earned by the player pursuant to the Contract at the time of the trade (excluding an Option Year if not yet exercised)… The specification of the amount… expressed as either (1) a specified percentage of the Base Compensation remaining to be earned… or (2) a specified dollar amount not to exceed a specified percentage of Base Compensation remaining to be earned… | Caps the bonus at 15% of remaining-to-be-earned Base Compensation (excluding unexercised Option Years) and fixes the two allowable amount forms | — | The basis is remaining-to-be-earned Base Compensation, with no 'guaranteed' qualifier — the prior canon wording was corrected by R3 |
...
| EV2-0034 | CBA2-A04.5 | CBA | SRC2-001 | — | CBA XXIV §2(a)(iii)(B)(3), p. 415; §2(a)(iv), pp. 415-16 | [Exhibit 4 may be amended] In connection with the trade of a Player Contract…, to reduce the amount of the trade bonus to be paid to the player (subject to Sections 2(a)(ii) and 2(a)(iii)(A) above)… A Contract that does not contain a trade bonus when signed cannot be amended to add one except [simultaneously with an Extension]… | Permits trade-connected reduction of an unearned bonus and bars additions outside the Extension pathways | — | — |
...
| EV2-0037 | CBA2-A04.8 | CBA | SRC2-001 | — | CBA VII §3(b)(1)(ii)-(b)(2), pp. 200-01; VII §6(j)(1), p. 240; VII §4(a)(1), p. 211 | The trade-earned bonus is a signing bonus allocated into the player's Salary for the then-current and remaining Salary Cap Years; §6(j) counts Replacement Players' post-assignment Salaries; §4(a)(1) includes the aggregate Salaries of all active players in Team Salary | The current-year allocation is part of post-assignment Salary, so it raises the assignee's §6(j) incoming value and Team Salary by composition of the three express rules | — | — |
...
| EV2-0067 | CBA2-A07.9 | CBA | SRC2-001 | — | CBA VII §8(e)(3), p. 263 | A Player Contract or Extension entered into pursuant to Section 8(e)(1) or (2) above may not contain an Exhibit 6 thereto. However, the preceding sentence shall not prohibit the Teams involved in the trade from agreeing that the trade… will be conditional upon the passage of a physical examination… | Bars the Exhibit 6 validity condition inside sign-and-trade/extend-and-trade contracts while permitting a physical-conditional trade | — | — |
| EV2-0068 | CBA2-A08.1 | CBA | SRC2-001 | — | CBA VII §8(a), p. 260 | …a Team shall be permitted to pay or receive in connection with one (1) or more trades occurring during a Salary Cap Year, directly or indirectly, up to an aggregate amount equal to 5.15% of the Salary Cap for such Salary Cap Year in cash across all such trades…; the amounts paid or received by a Team… shall not be netted against each other | Sets the aggregate annual paid-cash limit at 5.15% of the Salary Cap with the express no-netting rule (the section's own example exhausts both directions independently) | — | — |
| EV2-0069 | CBA2-A08.2 | CBA | SRC2-001 | — | CBA VII §8(a), p. 260 | …pay or receive… up to an aggregate amount equal to 5.15% of the Salary Cap… including cash received as reimbursement for Compensation obligations to players whom the Team is acquiring…; the amounts paid or received… shall not be netted against each other | Sets the aggregate annual received-cash limit (including reimbursements) with the no-netting rule | — | — |
...
| EV2-0071 | CBA2-A08.4 | CBA | SRC2-001 | — | CBA VII §8(a), p. 260 | …in connection with one (1) or more trades occurring during a Salary Cap Year, directly or indirectly… | Keys every charge to the Salary Cap Year in which the trade occurs, so conditional cash tied to a trade charges to the trade's year regardless of payment timing | — | The re-trade attribution/accounting mechanics for a later re-trade of a conditional asset are not expressed in the signed text and remain an unsupported operational candidate (§15.9.6) |
...
| EV2-0077 | CBA2-A10.4 | CBA | SRC2-001 | — | CBA VII §7(a)(3)(iii)(B), pp. 252-53 | …the Extension may, in the first Salary Cap Year covered by the extended term, provide for a Salary, excluding Incentive Compensation, of up to the greater of: (A) one hundred twenty percent (120%) of the Regular Salary in the last Salary Cap Year covered by the original term…; or (B) one hundred twenty percent (120%) of the Estimated Average Player Salary for the Salary Cap Year in which the Extension is signed (or, if the Extension provides for any Incentive Compensation…, then [120% of EAPS] less the amount of such Incentive Compensation)… Likely Bonuses and Unlikely Bonuses of up to one hundred twenty percent (120%)… | States the extension-and-trade first-year salary and bonus ceilings | — | Pre-2024-25 signings used the 105% rule of §7(a)(3)(iii)(A) (read this session) |
| EV2-0078 | CBA2-A10.5 | CBA | SRC2-001 | — | CBA VII §5(a)(4), p. 228 | For each Salary Cap Year covered by an Extension after the first Salary Cap Year covered by the extended term, the player's Salary, excluding Incentive Compensation, may increase or decrease… by no more than five percent (5%) of the Salary for the first Salary Cap Year covered by the extended term…; [per-bonus amounts] may increase or decrease… by up to five percent (5%)… | Fixes the 5% annual change limit for §8(e)(2) Extensions | — | — |
| EV2-0079 | CBA2-A10.6 | CBA | SRC2-001 | — | CBA VII §7(a) opening and §7(a)(1), pp. 249-50; §7(a)(2)(i)-(iii), pp. 250-51; §7(a)(3)(v), p. 253 | No Player Contract, other than a Rookie Scale Contract, may be extended except in accordance with the following… [anniversary and off-season timing rules; renegotiation and ETO-shortened-term bars; option exercise/non-exercise conditions]… a player who will not be a Qualifying Veteran Free Agent at the conclusion of his Contract will not be eligible to enter into an Extension pursuant to this Section 7(a) | Every §8(e)(2) Extension is a §7(a) Extension and must independently satisfy these rules | — | — |
...
| EV2-0084 | CBA2-A12.1 | INFERRED | SRC2-002 | — | BYL 4.01(a), pp. 62-63; BYL 4.02(a)-(b), pp. 63-64; BYL 4.05(a), p. 65 | Members may, by trade or sale, assign to other Members… draft choices in accordance with these By-Laws; the parties shall state their agreement on the terms and conditions of the transaction during a Trade Call…, including… all details with respect to… draft choices…; any term or condition… not disclosed… shall not be enforceable; a Member who has assigned its rights… may not retain any rights… | Assignment presupposes the assignor holds the conveyed selection right; enforceability requires the selections, years, and conditions to be stated on the Trade Call and in the Trade Memorandum | Inference chain: 4.01(a) assignment of owned choices + 4.02(a)-(b) full-terms disclosure/enforceability + 4.05(a) no retained rights → a team cannot convey a selection it does not own or an unstated expectation | The By-Laws do not state the ownership requirement in one sentence; this component records the inference boundary |
