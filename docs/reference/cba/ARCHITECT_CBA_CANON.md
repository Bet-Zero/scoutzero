...
**Current pre-R3.1 status (supersedes the R2.13 sequencing sentence above):**
...
accepted. R3.1 is now unblocked and is the next construction unit but remains
...
`d6101f82` by `/root/validation_scout`. None accepts an active A-series record.
Together the two compatibility acceptances unblock R3.1 as the next
construction unit; R3.1 remains not started, and R4 remains blocked until an
independent R3.1 checker ACCEPT. "Mechanical"
