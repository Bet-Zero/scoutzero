...
**Current R2.13 status (supersedes the R2.12 sequencing sentence above):**
R2.12 was executed at
`68db497240b22a997d472f67a62929358b81cc1e`, but its independent review
returned **REJECT/BLOCK-R3.1** because governed-ID pipe rows outside every
matching Inventory F range were silently ignored. R2.12 and its receipt are
immutable rejected historical evidence and unblock nothing. R2.13 closes
that one bounded mechanical defect with a generic whole-canon governed-row
location audit. R3 remains rejected, no A-series record is accepted, and
R3.1 remains blocked pending an independent R2.13 checker ACCEPT. Phase 1
remains open; Phase 2 and W1.1 remain blocked.
...
### 15.9 Register and source-certification standard (v2.0 — binding, R2.1 edition as hardened by R2.2 and corrected by R2.3–R2.13)
...
active §15.10–§15.12 record-table row. R2.13 accepts no active record and
does not start R3.1; only an independent R2.13 checker ACCEPT can unblock it.
...
R2.12 (executed; rejected) → R2.13 → independent R2.13 checker ACCEPT →
R3.1 → independent R3.1
checker ACCEPT → R4 →
independent R4 checker ACCEPT → R5 → independent R5 checker ACCEPT →
R6**. R4 depends on an independently **accepted** R2.13 foundation and
...
**R2.10–R2.13 dependent-gate propagation (binding; the repaired contracts flow
...
F intervals. **R9** independently re-runs all of the above.
...
The R2.10 and R2.11 review findings are classified exhaustively below.
R2.11 and R2.12 are immutable rejected history; R2.13 is a maker
checkpoint pending independent review and accepts nothing. "Mechanical"
