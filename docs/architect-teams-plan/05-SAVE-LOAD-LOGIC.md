...
**Note:** Do not read from `/players_v2` anywhere in Architect load/save. Architect's baseline data is exclusively `/architect/baseTeams` and `/architect/basePlayers`.
...
When loading `draftPicks`, keep the complete shape in memory (including `status`, `stepienEligible`, `tradeable`, `isSwap`, `swapDetails`, `dependsOn`, `conveyanceObligation`, `conditions`, `route`/`via`, and `metadata`). Downstream evaluators (e.g., Stepien checks) rely on these fields.
    ...
    async function signFreeAgent(worldId, teamCode, signingData) {
      ...
      const newPlayerContract = {
        ...
        contract: {
          ...
          signingDate: new Date().toISOString().split('T'),
      ...
      const newTeam = {
        ...
        deadCap: [
          ...(teamData.deadCap || []),
          {
            ...
            waiveDate: new Date().toISOString().split('T'),
            isStretched,
...
**Result:** Fast, efficient, and scalable! ✅F
