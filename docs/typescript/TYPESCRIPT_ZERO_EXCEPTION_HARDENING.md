...
| Mutation metadata probes | `(event.mutationMetadata as any)?.contractSummary` | DONE in TS-ZERO-003C-LOCAL-FIXTURE |
...
| --- | --- | --- | --- | --- | --- |
| TS-ZERO-003C-VALIDATION-TOOLS | Medium | Validation tooling | During the SDK/validator checkpoint, two `npm run typecheck` attempts hung without diagnostics and `npm run lint:md` failed because `markdownlint` is not installed in this checkout. During the local fixture and trade helper checkpoints, `npm run typecheck` completed but failed on unrelated existing diagnostics in `player-scrape`, `TeamSelectDropdown`, and the Firestore rules integration test setup. | Source/test target validation passed, but these checkpoints cannot be called fully typecheck/lint validated until these commands exit cleanly. | Re-run `npm run typecheck` and `npm run lint:md` after the unrelated root diagnostics and markdownlint dependency are available. |
