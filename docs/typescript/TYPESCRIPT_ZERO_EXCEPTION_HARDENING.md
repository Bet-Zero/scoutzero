...
- Next action: repeat the TS-ZERO-003C typed-harness pattern on the remaining
  ...
  (6) `entitlementWriter.collision.test.ts`,
  `entitlementResolver.parentFallback.test.ts`,
  `entitlementResolver.invariantViolation.test.ts` (typed `Firestore` DB
  sentinels instead of `{} as any`), plus strict-null fixes in
  `tradeContext_assertions.contract.test.ts` and
  `tradeEntitlementExclusivity.unavailable.test.ts` surfaced by `tsc`.
- Stop condition: the repo-wide test/mock escape scan has no practical central
  ...
  `any`/cast patterns.
- Last updated: 2026-04-29 (checkpoint 6 entitlement fake-DB sentinels)
...
| Gate 4 - Typed test fixtures and mocks | FAIL | TS-ZERO-003C partial checkpoints cleared fourteen Architect/guardrail/entitlements test files (including typed `Firestore` sentinels for entitlement writer collision and resolver harnesses); repo-wide scan still finds remaining entitlement identity move fake DB bags, dare mutator mocks, emulator gates, and other fixture bridges. | Continue Phase 3C before final closure. |
...
| TS-ZERO-003C | IN PROGRESS | Phase 3C: remaining test harness/mock scan clusters | Partial checkpoints cleared broad mock markers from fourteen Architect/entitlements test files, including typed `Firestore` DB sentinels for entitlement writer collision and resolver parent/invariant tests. Continue typed test-builder/mock hardening for remaining repo-wide scan hits (`entitlementIdentityMove`, dare mutator mocks, emulator gates). | Repo-wide test/mock escape scan; `npm run typecheck`; relevant scoped suites (`npm run test:architect -- --reporter=dot`, `npm run test:trade -- --reporter=dot`, `npm run test:node -- ...`, or `npm run test:diff -- --reporter=dot` when uncertain). | Prior partial packages plus `return_packages/typescript/TS-ZERO-003C-ENTITLEMENT-FAKE-DB-2026-04-29.md`; final TS-ZERO-003C return package still pending |
...
| 2026-04-29 | TS-ZERO-003C-ENT-DB | Replaced `{} as any` Firestore parameters with `{} as Firestore` in entitlement writer collision and entitlement resolver harness tests; aligned trade-context invalid snapshot and exclusivity assertions with strict `tsc`. | `src/tests/architect/entitlementWriter.collision.test.ts`; `src/tests/entitlements/entitlementResolver.parentFallback.test.ts`; `src/tests/entitlements/entitlementResolver.invariantViolation.test.ts`; `src/tests/architect/tradeContext_assertions.contract.test.ts`; `src/tests/architect/tradeEntitlementExclusivity.unavailable.test.ts`; plan doc; return package | `npm run typecheck` PASS; `npm run test:node -- --reporter=dot` on five touched test files PASS (18 tests). | `return_packages/typescript/TS-ZERO-003C-ENTITLEMENT-FAKE-DB-2026-04-29.md` |
...
| Entitlements / entitlement writer | `fakeDb = {} as any`; `(failedTeam as any).rules` | ~~`tradeEntitlementExclusivity.unavailable`~~ (typed `TradeTeamResult`; 2026-04-29); ~~`entitlementWriter.collision.test.ts`~~, ~~`entitlements/*Resolver*.test.ts`~~ (typed `Firestore` sentinels, 2026-04-29); remaining: `entitlementIdentityMove`, other entitlement integration mocks |
...
| 2026-04-29 | `npm run typecheck` | PASS | Root `tsc --noEmit` completed after entitlement fake-DB and strict-null test edits. |
| 2026-04-29 | `npm run test:node -- --reporter=dot` (five targeted test files) | PASS | 18 tests across entitlement writer collision, both entitlement resolver suites, trade-context assertions, and trade exclusivity unavailable. |
| 2026-04-29 | `npm run lint:md` | PASS | Markdownlint exited 0 after updating `docs/typescript/TYPESCRIPT_ZERO_EXCEPTION_HARDENING.md` (script still walks broader doc globs per package.json). |
...
| 2026-04-29 | TS-ZERO-003C-ENT-DB | `return_packages/typescript/TS-ZERO-003C-ENTITLEMENT-FAKE-DB-2026-04-29.md` | Phase 3C entitlement writer collision and resolver `Firestore` sentinel hardening evidence. |
