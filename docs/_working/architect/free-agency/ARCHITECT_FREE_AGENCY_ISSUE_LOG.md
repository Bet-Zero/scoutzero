...
| FA-2-3 | FA-2C, FA-2D | MEDIUM | Visible identity / lookup truth is materially tighter in live code. `FreeAgentPool.tsx` now builds one `FreeAgentSurfaceEntry` from runtime `playersMap`, tracks selection and row-menu state by `selectionKey`, and stages modal launch from that same shared entry, which removes the old `playersById` seam and name-only selection drift. Focused runtime/static guardrails now pin duplicate-name selection independence plus stable id lookup precedence. The combined issue remains OPEN because Step 2D still needs to finish the broader guardrail closure for this root problem. | OPEN |
