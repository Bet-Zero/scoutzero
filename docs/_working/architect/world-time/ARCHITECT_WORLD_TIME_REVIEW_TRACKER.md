...
| WT-F2A | Guard Season-Advance Success Callbacks Against Active-World Identity Drift | DONE | `useArchitectState.ts` now exposes an active-world identity token, `GMDashboard.tsx` fans that token into `OffseasonSection.tsx`, and the section drops stale modal completions before staging committed aftermath or calling the shared reload seam. |
| WT-F2B | Make Standard-Signing Stale Reload Rejection Explicit and Fail-Closed    | DONE   | `reloadActiveWorldTeamData(...)` now returns explicit `applied` vs `stale-drop` outcomes, and `useArchitectActions.ts` only applies local post-signing `freeAgents` aftermath after a committed world reapply actually lands. |
