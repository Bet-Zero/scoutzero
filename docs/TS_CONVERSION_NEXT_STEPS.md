...
Updated 2026-04-18: `src/features/table/hooks/useFilterDiagnostics.js` → `useFilterDiagnostics.ts` with typed diagnostics payloads, diagnostic player/option coverage shapes, and query-param gating. Typecheck surfaced stale uppercase catalog-status comparisons; the hook now counts typed lowercase catalog statuses and maps them to the panel's uppercase display statuses. Added focused coverage at `src/tests/table/useFilterDiagnostics.test.tsx`. Validation for this slice: `npm run typecheck`, `npm run validate:project`, and `npm run test:ui -- --reporter=dot src/tests/table`.
Updated 2026-04-18: `src/features/profile/hooks/usePlayerNavigation.js` → `usePlayerNavigation.ts` with typed route params, player lookup state, keyboard guard targets, navigation callbacks, and an exported hook result shape. The route resolver now feeds through `getPlayerRouteId()` before reading the full typed player row from the hook's `playersData` map. Validation for this slice: `npm run typecheck`, `npm run validate:project`, and `npm run test:scouting -- --reporter=dot`.
Updated 2026-04-18: `src/features/profile/hooks/usePlayerProfileState.js` → `usePlayerProfileState.ts` with typed evaluation state, modal save payloads, dirty-state handlers, and focused hook smoke coverage. The conversion made normalized role hydration explicit via `primaryEvaluation.roles` from `enrichPlayerData`. Validation for this slice: `npm run typecheck`, `npm run validate:project`, and `npm run test:scouting -- --reporter=dot`.
Updated 2026-04-18: `src/features/profile/hooks/useAutoSavePlayer.js` → `useAutoSavePlayer.ts` with typed autosave snapshots, `saveNow` overrides, save-state results, and an unknown-safe Firestore write sanitizer. Added focused hook coverage that verifies `evaluations/current`, current-season `evaluationView`, and main-doc `currentEvaluationView` writes through the Firestore mock. Validation for this slice: `npm run typecheck`, `npm run validate:project`, and `npm run test:scouting -- --reporter=dot`.
Updated 2026-04-18: `src/features/roster/hooks/useRosterManager.js` → `useRosterManager.ts` with typed roster state, selected team state, processed drawer players, saved roster projects, slot actions, and serialized roster project player ids for the typed Firestore helpers. Existing saved-roster UI coverage exercises deep-link loading, missing-player placeholders, full-roster disablement, and overwrite payloads. Validation for this slice: `npm run typecheck`, `npm run validate:project`, `npm run test:roster -- --reporter=dot`, and `npm run test:ui -- --reporter=dot tests/roster/rosterBuilder.ui.test.jsx`.
Updated 2026-04-18: `src/features/ranker/hooks/useRankerSession.js` → `useRankerSession.ts` with typed local draft state, hydrated closure-cache drafts, owner-only Firestore methods, save-as-list state, and explicit error narrowing. Added focused hook coverage for local draft hydration and explicit owner Firestore saves. Validation for this slice: `npm run typecheck`, `npm run validate:project`, `npm run test:node -- --reporter=dot tests/rankingEngine.test.js tests/rankerLocalDraft.test.js tests/rankerSessionSerialization.test.js tests/rankerSaveAsList.test.js src/tests/ranker/rankerHelpers.smoke.test.ts`, and `npm run test:ui -- --reporter=dot src/tests/ranker/useRankerSession.test.tsx`.
