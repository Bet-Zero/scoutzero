...
- Active checklist: `docs/_working/workspace-cleanup/SCOUTZERO_WORKSPACE_CLEANUP_HISTORICAL_DOCS_ADJUDICATION_CHECKLIST.md`
- Review rule: only re-point a reference when the target exists in the current
  workspace; otherwise use `ANNOTATE_MISSING`, `KEEP_LITERAL`, or `ESCALATE`.
- Intended use: work the five carried-forward docs one at a time without
  broadening the workspace-cleanup scope.
