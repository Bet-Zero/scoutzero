# RETURN PACKAGE: CAP SHEET MASTER DOC NORMALIZATION

**Date:** 2026-01-16
**Mode:** Docs-Only Normalization

## 1. CHANGES APPLIED

- **Markdown Normalization:**
  - Fixed "SSOT Output Contract" code fence (was creating run-on code block).
  - Ensured valid Markdown tables (spacing/pipes).
  - Enforced strict headers for sections G–K.
- **Path Verification:**
  - Normalized `CapSheet` path to: `src/features/architect/capSheet/CapSheet/CapSheet.jsx` (Verified exists).
  - Confirmed SSOT logic path: `src/features/architect/utils/capTotals/computeTeamCapTotals.js`.
- **Content Updates:**
  - **Deduplication:** Merged/removed duplicate "Phase 0 findings" into single canonical section.
  - **SSOT Refinement:** Explicitly labeled `TeamCapTotals` fields as "Observed" from `computeTeamCapTotals.js`.
  - **Clarification:** Updated `worldlessBaselineSalary.js` recommendation to note it is unverified in product code (conceptually verified as unused in key files, but marked as "needs verification" per instructions if not fully proven). *Correction: I verified it is unused in TradeTeamCard and CapImpactTiles, but per rules I will be precise.*

## 2. VERIFICATION LOG

- **Paths Checked:**
  - `src/features/architect/capSheet/CapSheet/CapSheet.jsx` (EXISTS)
  - `src/features/architect/utils/capTotals/computeTeamCapTotals.js` (EXISTS)
  - `src/features/architect/utils/worldlessBaselineSalary.js` (EXISTS, Unused in TradeTeamCard/CapImpactTiles)
- **Render Check:**
  - Verified structure closes code blocks before headers.

## 3. FINAL DOC CONTENTS (Copy-Paste Ready)

*(Content below is the normalized docs/architect/CAP_SHEET_MASTER_DOC.md)*
...
| :--- | :--- | :--- | :--- | :--- |
| **Cap Sheet** | `CapSummaryTiles` | Room, Aprons, Total | `TeamCapTotals` | Currently wired correctly? |
| **Cap Sheet** | `CapSheet` (Grid) | Totals Row | `TeamCapTotals` | **VIOLATION:** Computes locally via `calculateCapHitTotal`. |
| **Trade Machine** | `CapImpactTiles` | Room, Aprons | `TeamCapTotals.deltas` | Wired correctly (uses SSOT). |
| **Trade Machine** | `TradeTeamCard` | Total Salary | `TeamCapTotals` | **VIOLATION:** Uses `teamTotalSalary` or `getSalaryForYear`. |
...
| :--- | :--- | :--- |
| **High** | `src/features/architect/utils/salaryUtils.js` | `payrollForYearFromCapSheet` (Duplicate of SSOT) |
| **Medium** | `src/features/architect/utils/salaryUtils.js` | `deadMoneyForYear` (Duplicate of SSOT) |
| **High** | `src/features/architect/hooks/useTradeMachine.js` | `payrollForYearFromCapSheet` (Local inline duplicate) |
| **Medium** | `src/features/architect/hooks/useTradeMachine.js` | `deadMoneyForYear` (Local inline duplicate) |
| **High** | `src/features/architect/utils/worldlessBaselineSalary.js` | `getWorldlessTeamBaselineTotal` (Redundant parallel SSOT) |
| **High** | `src/features/architect/capSheet/CapSheet/CapSheet.jsx` | `calculateCapHitTotal` (UI-level duplicate math) |
...
2. **`worldlessBaselineSalary.js` Disposition:**
    - **Recommendation: DELETE (Evidence: Verified unused in `TradeTeamCard.jsx`, `CapImpactTiles.jsx`. Only found in tests. Pending final confirmation of other potential call sites).**
    - This file is redundant. `computeTeamCapTotals.js` safely handles data without World wrappers if passed a basic team object. We should not maintain two parallel math engines just because the *loading* strategy differs.
