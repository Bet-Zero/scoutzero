...
* * 2026-02-28: CAP_SHEET_E2E_SSOT_PARITY_E1 - Cap Sheet SSOT Parity Enforcement (EXECUTION) - Closed 4 preflight STOP triggers for cap sheet total consistency. (1) Replaced `CapSheetFull.jsx` local yearTotals math (which missed dead money and incomplete roster charges) with per-year `computeTeamCapTotals()` calls via `useMemo`. (2) Fixed `ManageDeadMoneyModal.jsx` to write canonical `deadCap.amountByYear` array shape (`[{ season, amount, isStretched? }]`) per `DeadCapItemZ` schema instead of object-map; read path handles both shapes for backward compat; removed 4 non-allowlisted fields (`label`, `stretched`, `buyout`, `reason`). (3) Added Bridge Gate to `seasonManager.js` season advance write path: `sanitizeTransientFieldsForPersistence()` + `assertPersistableOrThrow()` now run before batch.set(), matching `persistWorldMutation()` hygiene order (sanitize → normalize TPE → validate contract → removeUndefined → write). (4) Roster shape confirmed safe: `computeTeamCapTotals` uses `team.players` (hydrated objects), not `team.roster`. 20 new guardrail tests across 3 test files. Return package: `return_packages/cap_sheet/CAP_SHEET_E2E_SSOT_PARITY_E1_EXECUTION_RETURN_PACKAGE.md`.
*
* LINKS:
