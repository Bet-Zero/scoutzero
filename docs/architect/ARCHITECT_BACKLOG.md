...
| TEST-002 | Testing | Tech-debt | HIGH | The `architect-qa` e2e suite was **rotted against the cockpit refactor** (waited for old `#world-selector`; world controls moved to `cockpit/WorldMenu.tsx` popover; nav is `role="tab"` not `button`). 🔧 Partially fixed: `openDashboardTab` → `role=tab`; added `openWorldMenu`/`closeWorldMenu` and rewired the world helpers. Now **D-MQ-004 (trade validation fail-closed), D-MQ-006 (offseason preview), and the smoke tests pass** (these flows verified). Worlds create/select correctly. **Still failing:** D-MQ-002 (world-date +1), D-MQ-003 (full persisted trade marathon), D-MQ-007 (season-advance modal) **time out** — likely remaining stale selectors in the trade/season helpers (`addTradeTeam`/`routeTradePlayer`/season-advance) and/or tight timeouts for the create-world+topup marathon in the emulator. Next: trace those three to green. | OPEN |
