...
| DATA-003 | Cap data | Data | MED | League cap/apron/tax are hand-typed constants (`capProjections.ts`); 2026-27 still projected — NBA sets official ~July 1. Update when released. ✅ **DONE (Jul 2026, BZE-197):** official 2026-27 numbers entered (cap $164.961M, tax $200.428M, aprons $209.015M/$221.686M, floor $148.465M, MLEs/BAE per NBA release) and `confirmed: true`. All figures came in slightly *below* the projections. Note: NBA now projects ~5.5% growth for 2027-28 (~$174M) vs our projected $182M (growthRate 0.1) — re-deriving 2027-28+ projections is a separate decision. | DONE |
| ENG-003 | Schema | Tech-debt | LOW | `BaseTeamDocZ` doesn't yet include ledger-derived fields (`draftPicksInventory/Obligations/Contested`, `draftAssets`); stripped before validation (`stage_team.ts:1174`). 🔎 **Reviewed Jun 2026 — deferred:** gated on its own precondition ("once stable and the Trade Machine is wired to use them") and it edits the **critical team-data staging validation**. Adding the Zod shapes must be verified against real staged data via the scrape/stage pipeline (headed session) — a wrong shape breaks staging. Do it in a session that can run staging, not blind. | OPEN |
| TEST-001 | Testing | Tech-debt | MED | Broad-Architect test debt predating Next Era (closure gates, migration-phase guardrails, one mock-gap test) — flagged in Stage 6 ship audit as "track separately." Audit + close. 🔎 **AUDITED (Jun 2026):** node architect suite is **green** (3459/3459, after fixing the one SBX-001 handoff-guardrail). The debt lives in the **UI suite**: `vitest run -c vitest.ui.config.js src/tests/architect` → **111 failed / 783 passed across 30 files** (all pre-existing, from the cockpit/Next-Era refactor — confirmed unrelated to recent work). Root causes: (1) **refactor-rotted `vi.mock` factories** — factory doesn't return an export the refactored component now imports (dominant: `useCapValidation`, `EditContractModal`, `SeasonAdvanceModal`, `toSeasonKey`, `CapSheet`, `FreeAgentPool`, `DraftPositionsInput`); (2) **component role/text-finder rot** (e.g. offer-sheet "Offer Sheet" checkbox, "TOTAL CAP ALLOCATIONS" text); (3) **module-export structural assertions** (`['OffseasonTab'] vs ['default']`). Spread thin (≤12 failures/file), no single high-leverage fix. ⚠️ **Close as a dedicated PR, not a rushed pass** — some failures (esp. the role/text ones) may be **real regressions** from the refactor, so each needs per-test judgment before "making it green" to avoid masking a real break. ✅ **RESOLVED (Jun 2026):** UI suite now **green (894/894, 101 files)**; node suite still **green (3461/3461)**; `tsc --noEmit` clean. Triaged Category 2 first and found **two real regressions, both fixed in production code**: (a) **EditContractModal** — the form-hook extraction (8767faec) dropped `setSelectedAction(initialAction)`, so opening the modal from a clicked cap-table cell / sign-and-trade / waive-stretch landed on the action picker instead of the action's form; restored as a dedicated effect. (b) **CapSheetFull** — the 2K visual redesign (137d9acc) turned the canonical-totals `<section>` into a `<div>`, dropping its `role="region"` a11y landmark and merging its sr-only descriptions; restored role + split copy (no visual change). Everything else was rot from intentional refactors, updated to current reality: refactor-rotted `vi.mock` factories (default→named export conversion 10f5fed7); **cockpit Phase-2A migration** (canonical totals summary + hard-cap badge moved from the cap sheet to the persistent `TeamStatusStrip`; world controls behind an explicit popover; nav rail items are `role="tab"`); **deep-link nav** (`useArchitectDeskNavigation` reads `?season=/?room=/?player=`, so League View "Manage Team" now routes `/gm/<slug>?season=&room=roster` — path = identity, query = entry hints); **modal redesigns** (ManageDeadMoney/ManageExceptions copy + structure; exception amounts now CBA-fixed); **hook splits** (useArchitectActions + useArchitectState into satellites); and **export-shape parity** guardrails realigned to named exports. No offer-sheet behavior regressions (ENG-004 already fixed; those tests were mock-cascade rot). | RESOLVED |
...
| TEST-003 | Testing | Tech-debt | MED | **e2e flakiness on the automated create-world marathon** (D-MQ-002/003/007/008). ✅ **DONE** — full `architect-qa` suite is now **13/13 green** in review mode. Root unknown resolved: worlds are owner-scoped (`listUserWorlds` filters `createdBy == uid`) and review mode signs in anonymously with a *dynamic uid*, so a static boot-seed is invisible to the test user. BZE-36 later changed the rules so owner-scoped `architect_worlds` list queries are allowed from world metadata; the e2e helper still uses active-world localStorage restore for deterministic reload coverage. **Fix (Option C, test-only):** `ensureWorldSelected` reads the live anon uid from Firebase auth IndexedDB, admin-seeds a world owned by it (with `asOfDate`), and activates it via the app's own localStorage rehydration path — no app/auth behavior changes. Fixing 003/008/005/009 also required repairing **post-cockpit selector rot** newly exposed once worlds worked (see TEST-002): trade-card "Plyr (N)" label + dual wide/compact layouts + dialog scoping + auto-included current team; renamed cockpit re-entry link; smoke "Full Cap Table" region; and an offer-sheet RFA home-team world snapshot. **Also fixed a real product bug** → see ENG-004. | DONE |
...
| SBX-001 | Offseason / Compare | UX | MED | In Sandbox (no world), Offseason (Advance Season, draft positions) and Compare are fully gated to "select a world" with no in-screen way to create/pick one — feels like a dead end. Add a path to create/select a world from these screens. ✅ **DONE** — the gated empty-states now render the shared `WorldSelector` inline (create via "+ New" + pick existing), wired from GMDashboard via a `worldPickerSlot` prop on both sections (reuses the existing control, no duplicated world logic). Verified by e2e (`SBX-001` block, 2 tests). | DONE |
| UX-001 | Dashboard | UX/Polish | LOW | Large empty canvas below the cap table with a small roster. Verify with a full 15-man roster; if still sparse, consider filling the space. 🔎 **Reviewed Jun 2026 — needs a human:** this is a visual/layout judgment ("if still sparse, consider…") that can't be assessed headlessly. Best done by eye against a full 15-man roster in review mode. ✅ **DONE (Jul 2026, BZE-194):** owner reviewed before/after screenshots and approved Option A — sparse-roster (≤5 cards) vertical rhythm tightened in `RosterVisual.tsx`; cards sit higher, title/background treatment quieter, full-roster rendering untouched. Commit `7f5db926`. | DONE |
...
**The real unknown, resolved:** worlds are owner-scoped and review mode signs in
anonymously with a dynamic per-session uid, so a static boot-seed is invisible to
the test user. BZE-36 later changed the world metadata rules so owner-scoped
`architect_worlds` list queries are allowed when constrained by `createdBy`.
The e2e helper still rehydrates the active world from `localStorage` because
that directly covers the reload path the app uses after a user selects a world.

**Fix shape (Option C, test-only):** `ensureWorldSelected` reads the live anon
uid from Firebase auth IndexedDB, admin-seeds a world owned by it (with an
`asOfDate` so the world is genuinely ready), writes the active-world localStorage
key the way the app does, and reloads so `useArchitectState` restores it.
