...
| CS-3B | Fix Future-Only Player Visibility Risk in the Multi-Year Table                                                               | IN_PROGRESS | `CapSheetFull.tsx` now builds the visible multi-year player body from the displayed seven-year contract window instead of filtering only on current-year slices, so future-only contract contributors render in the table body while cap holds remain separate. Focused UI and source-scan/canonical guardrails now pin future-only visibility and future-total coherence. Required `typecheck` and `build` passed; required `test:diff` finished with unrelated pre-existing Architect failures outside CS-3B scope, so execution remains in progress pending clean repo-level validation. |
