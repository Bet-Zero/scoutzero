...
function applyPlaceholders(str, placeholders) {
  return str
    .replaceAll('{seasonId}', placeholders.seasonId)
...
function mapDoc(legacy, mapping) {
  ...
  for (const row of mapping.mappings) {
    try {
      ...
      const transformFn =
        transforms[row.transform || 'copy'] || transforms.copy;
...
function validateEdgeCases(targetDoc, subcollections, playerId) {
  ...
  if (
    targetDoc.bio?.display?.freeAgentYear &&
...
async function processBatch(batch, docs, mapping, dstCollection, isDryRun) {
  ...
  for (const doc of docs) {
    ...
    // Check edge cases
    const edgeCaseIssues = validateEdgeCases(
      targetDoc,
      subcollections,
      playerId
    );
    ...
    if (warnings.length > 0) {
      console.log(
        `⚠️  WARNINGS ${playerId} (${warnings.length}):`,
    ...
    if (isDryRun) {
      ...
      if (subcollections.contracts) {
        Object.entries(subcollections.contracts).forEach(
      ...
      if (subcollections.seasons) {
        Object.entries(subcollections.seasons).forEach(
          ([seasonId, seasonData]) => {
            batchOps.push({
              ref: dstCollection
                .doc(playerId)
      ...
      if (
        ...
        Object.keys(subcollections.evaluations).length > 0
      ) {
        batchOps.push({
          ref: dstCollection
            .doc(playerId)
  ...
  if (!isDryRun && batchOps.length > 0) {
    ...
    await retryWithBackoff(async () => {
      await batch.commit();
      console.log(
        `✅ Batch committed: ${batchOps.length} documents/subcollections`
      );
    });
  }
...
async function main() {
  ...
  console.log(
    `   Mode: ${args['dry-run'] ? 'DRY RUN' : args.shadow ? 'SHADOW' : 'LIVE'}`
  );
  console.log('');
  ...
  for (let i = 0; i < docs.length; i += PLAYERS_PER_BATCH) {
    ...
    console.log(
      `📦 Batch ${batchNum}/${totalBatches} (${batchDocs.length} players):`
    );
