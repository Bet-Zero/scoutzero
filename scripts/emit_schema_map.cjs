...
// Input: ./_exports/current_export.json (from your export script)
const fs = require('fs');
...
function main() {
  ...
  const outJson = path.join('_exports', `schema_map_${SEASON}.json`);
  const outCsv = path.join('_exports', `schema_map_${SEASON}.csv`);
