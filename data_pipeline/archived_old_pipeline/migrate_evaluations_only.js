...
try {
  ...
  console.log(
    '⚠️  Firebase credentials not available, will create sample evaluations structure'
  );
  hasFirebase = false;
}
...
class EvaluationOnlyMigrator {
  ...
  async migrateEvaluationsOnly() {
    ...
    try {
      ...
      console.log(
        `✅ Loaded ${existingData.length} players from your database`
      );
  ...
  async loadExistingPlayerData() {
    ...
    if (!hasFirebase) {
      throw new Error(
        'Firebase credentials not available. Cannot access existing player data.'
      );
    }
  ...
  async preserveUserEvaluations(players) {
    ...
    if (!hasFirebase) {
      console.log(
        '⚠️  No Firebase connection - creating sample evaluation structure'
      );
    ...
    for (const player of players) {
      ...
      if (evaluationData) {
        ...
        this.results.playersWithEvaluations.push({
          ...
          id: player.id,
          evaluations: Object.keys(evaluationData).filter(
            (key) => !['evaluator', 'last_updated', 'source'].includes(key)
          ),
        });
      }
      ...
      if (evaluationsCount % 400 === 0 && evaluationsCount > 0) {
        console.log(
          `💾 Committing batch at ${evaluationsCount} evaluations...`
        );
        await evaluationsBatch.commit();
        // Create new batch
        const newBatch = db.batch();
        Object.assign(evaluationsBatch, newBatch);
      }
    }
  ...
  extractEvaluationData(player) {
    ...
    if (
      player.subRoles &&
      (player.subRoles.offense?.length || player.subRoles.defense?.length)
    ) {
      evaluationData.subRoles = player.subRoles;
      hasEvaluationData = true;
    }

    // Badges
    if (player.badges && player.badges.length > 0) {
      evaluationData.badges = player.badges;
      hasEvaluationData = true;
    }

    // Blurbs (custom scouting notes)
    if (player.blurbs && Object.keys(player.blurbs).length > 0) {
      evaluationData.blurbs = player.blurbs;
      hasEvaluationData = true;
    }

    // Shooting profile
    if (player.shootingProfile && player.shootingProfile !== '—') {
      evaluationData.shootingProfile = player.shootingProfile;
      hasEvaluationData = true;
    }

    // Status or tier
    if (player.status || player.tier) {
      evaluationData.status = player.status;
      evaluationData.tier = player.tier;
      hasEvaluationData = true;
    }
